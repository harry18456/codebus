...
Defines how codebus's Windows installer is built and published automatically: a GitHub Actions workflow that, on a `v*` tag push or manual dispatch, reproduces the local `tauri build` path on a `windows-latest` runner via `tauri-apps/tauri-action`, then attaches the unsigned NSIS `-setup.exe` to a draft GitHub Release held for human review before publishing.
