...
codebus's push/PR continuous-integration contract: trigger conditions, the `windows-latest` runner/toolchain, the Rust and frontend validation commands, and the clippy baseline guard that enforces "no new warnings" without `-D warnings`. Distinct from `release-automation`, which owns the tag-triggered installer publish path.
...
<!-- @trace
source: github-ci-and-templates
updated: 2026-06-16
code:
