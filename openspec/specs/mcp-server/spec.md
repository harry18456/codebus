...
Expose a codebus vault's wiki to external MCP-compatible agents (Claude Code, Codex) over a stdio Model Context Protocol server — query-only, tools-only. A single `codebus mcp --vault <path>` process serves exactly one vault; cross-project access is achieved by configuring multiple server entries in the client's MCP config.
