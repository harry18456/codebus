...
The sidecar SHALL implement an async `run_generator(*, state, llm_chat_provider, kb, options) -> GeneratorResult` function in `codebus_agent.generator.runner` that consumes an `ExplorerResult` (specifically `state.stations`) plus a `KnowledgeBase` instance plus a `task: str` plus `options(mode, target_persona)`, and returns a `GeneratorResult(tutorial_path, station_paths, route_path, log_path, degraded_count)`. The function MUST iterate over `state.stations` in order, invoking `_generate_station(station, idx, context)` for each one, then assemble the MOC and write `route.json` after all stations have been written (or marked degraded).
...
- **WHEN** `run_generator(state=<3 stations>, llm_chat_provider=<factory returning scripted MockProvider>, kb=<empty stub>, options={"mode":"interactive","target_persona":"experienced engineer"})` is called against a fresh workspace
...
5. Total markdown character count (excluding frontmatter) MUST NOT exceed 800. Issue: `"too_long"`.
...
- **WHEN** `validate_station_markdown(md="# Title\n\nSome text.", station_idx=2, mode="interactive", workspace_root=<path>)` is called
- **THEN** the returned `issues` list MUST contain `"missing_checkpoint"`

#### Scenario: Interactive mode rejects two quizzes

- **WHEN** the validator receives markdown containing two `<Quiz>` elements
- **THEN** the returned `issues` list MUST contain `"too_many_quizzes"`

#### Scenario: Quiz with invalid correct attribute is rejected
...
#### Scenario: Length over 800 characters fails validation

- **WHEN** the validator receives markdown whose body (excluding frontmatter) has 1500 characters
- **THEN** the returned `issues` list MUST contain `"too_long"`

#### Scenario: CodeRef pointing outside workspace fails validation

- **WHEN** the validator receives markdown containing `<CodeRef file="../../etc/passwd" lines="1-10" />` against `workspace_root=/tmp/ws`
- **THEN** the returned `issues` list MUST contain `"coderef_escape: ../../etc/passwd"`

#### Scenario: Plain mode tolerates absence of components

- **WHEN** `validate_station_markdown(md="# Title\n\nPlain prose without any custom elements.", station_idx=1, mode="plain", workspace_root=<path>)` is called
- **THEN** the returned `issues` list MUST NOT contain `"missing_checkpoint"`, `"too_many_quizzes"`, or `"quiz_bad_correct: ..."` issues
