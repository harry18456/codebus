...
- **THEN** the parse SHALL succeed AND the resulting `RunLog.outcome` SHALL equal the string `"succeeded"` (the serde default value) AND the resulting `RunLog.session_id` SHALL equal `None` AND the resulting `RunLog.interrupt_reason` SHALL equal `None`
