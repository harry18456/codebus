...
- 新建 `openspec/specs/interactive-tutorial/` capability，6 個 ADDED Requirements：
  - 站牌路由 schema（`/tutorial/{workspace_id}/{station_id}` 與 `/tutorial/{workspace_id}/index` MOC）以 stable station id 為 URL key（禁用 numeric index）
...
- 修改 `web/app/pages/workspace/grant.vue`：grant 成功後 `router.push('/workspace/scan')` 改為 `/tutorial/{workspace_id}/index`，用 `GrantResponse.workspace_id` 直接帶入路由
...
- Affected code:
  ...
  - Modified:
    ...
    - web/app/pages/workspace/grant.vue（granted 後跳 `/tutorial/{workspace_id}/index`）
