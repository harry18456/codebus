...
- R-01 路由 schema：`/tutorial/{workspace_id}`（MOC）+ `/tutorial/{workspace_id}/{station_id}`（單站）；URL key 一律用 D-029 stable station id（Nuxt 4 file routing 慣例：`pages/tutorial/[workspace_id]/index.vue` 對應 bare `/tutorial/:workspace_id`，**無** `/index` segment）
...
- grant.vue granted 後跳 `/tutorial/{workspace_id}`
...
4. grant.vue 修改 redirect target（從 `/workspace/scan` 改 `/tutorial/{workspace_id}` — bare path，無 `/index` segment）
