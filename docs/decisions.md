...
- [x] Tauri ↔ Python HTTP IPC schema 草案（`docs/sidecar-api.md` + `openspec/specs/sidecar-runtime/spec.md`，2026-04-19 m1-power-on 落地）
- [x] PyInstaller 打包流程驗證（`openspec/specs/app-packaging/spec.md` + `sidecar/codebus-sidecar.spec`，2026-04-19 m1-power-on 落地）
...
**後續**
- [x] 設計 LLM Provider 抽象介面（Python Protocol / ABC）— M1 power-on archive 時落地
- [ ] Phase 2 評估 Rig 是否原生支援 Ollama（若屆時已 port 到 Rust）
- [ ] Phase 2 評估 Ollama 本地模型品質（對 golden sample 跑一次看召回/雜訊）

**附註**
- 2026-04-20：role routing 落地（見 `openspec/changes/llm-role-routing/`）— `ProviderRegistry` 改為 `ProviderRole → TrackedProvider` map，四個固定 role（REASONING / JUDGE / CHAT / EMBED）；Phase 2 切 Ollama 只需在 config 改 `provider_id`，registry 結構不動
...
- [x] 起草 golden sample ideal route（`tests/golden/timeline-gdrive-adapter/ideal-route.md`，2026-04-17）
- [x] 決定放在 `tests/golden/timeline-gdrive-adapter/` 結構（已存在，2026-04-17）
...
**後續**
- [x] README「與現有工具差異化」對照表加入這四項（2026-04-17 隨 D-005 狀態整體完成）
- [x] 常見問答章節加一題「跟 DeepWiki 差在哪」（2026-04-17 隨 D-005 狀態整體完成）

---

## D-006: Golden sample 評估機制

**狀態**：✅ 已補進 README（2026-04-17）；✅ harness 落地 P0 形狀（2026-04-24，`explorer-judge-golden`）；✅ scoring helpers + Timeline synthetic fixture 落地（2026-04-25，`golden-sample-baseline`）；完整評估 rubric（含 live LLM snapshot）落地延後至打磨期
**討論日期**：2026-04-17
...
**Golden harness 落地形狀**（`explorer-judge-golden`，2026-04-24）

首份 regression harness 只鎖**結構性行為**，不鎖「教材品質」—— 後者留給打磨期真 LLM snapshot。

- **Driver**：scripted `MockProvider`（reasoning 與 judge 各一個 `MockScript`），隔絕 LLM 層不確定性。
- **Fixture**：單一 `tests/golden/demo-synthetic/`（P0 只 pin 1 份；`timeline-gdrive-adapter/` 延後）；workspace 只需 dummy `.py` 讓 Scanner/Explorer 的 path resolve 不炸，內容不影響 replay 結果。
- **Baseline 檔**：`tests/golden/demo-synthetic/expected.json` 鎖 5 欄——`stations: [{path, role}]` / `stopped_reason ∈ {budget_exhausted, queue_empty, cancelled}` / `step_count` / `judge_prompt_version` / `explorer_prompt_version`。
- **比對粒度**：stations 按 `(path, role)` pair 做 **set equality**（不比 `relevance` / `why` / `depends_on`——這三者對 Judge prompt 文字變動過度敏感）；`stopped_reason` / `step_count` / 兩個 prompt_version 做 equality。
- **Harness 位置**：`sidecar/tests/golden/test_explorer_replay.py`，fixture 路徑用 `Path(__file__).resolve().parents[3]` 絕對解析，不依賴 pytest cwd。
- **Drift guards**：錯誤訊息要包含 (a) 差異的 `(path, role)` 字串、(b) `re-baseline required` hint（version drift 時）—— 強迫實作者動手重 pin baseline 而不是靜默 skip。
- **Re-baseline 觸發機制**：`JUDGE_SYSTEM` / `render_judge_prompt` / `JUDGE_PROMPT_VERSION` / `EXPLORER_PROMPT_VERSION` 任一異動 → harness 炸 → 必須在同一 commit 更新 `expected.json` 對應欄位。

**後續**
- [x] spec 裡已有（`agent-explorer-spec.md` 十一、評估方式），README 補一段提到這機制（2026-04-17 隨 D-006 狀態整體完成）
- [x] 建立 `tests/golden/` 放 ideal routes（`demo-synthetic/` 落地 2026-04-24 via `explorer-judge-golden`；`timeline-storage-adapter-synthetic/` 落地 2026-04-25 via `golden-sample-baseline`）
- [x] scoring 公式落地（`station_recall` / `station_noise` / `composite_score` / `IdealRoute` schema 在 `sidecar/tests/golden/scoring.py`，2026-04-25 via `golden-sample-baseline`；`depth` 暫回 `1.0` placeholder 等 Module 5 station depends_on 真實填值）
- [ ] 打磨期：接真 LLM snapshot replay（`@pytest.mark.live_llm` + skip-by-default），評估教材品質而非結構
...
**後續**
- [ ] 三階段進度元件 Vue 元件 spec（前端 Stage 6 開鋸時動）
- [x] Python sidecar emit progress event 到 Tauri → 前端的 schema（`sse-progress-skeleton` 2026-04-22 + `agent-sse-wiring` 2026-04-24 落地，`docs/sidecar-api.md §四` 完整列 `agent_thought` / `agent_action_result` / `judge_verdict` / `progress` / `usage_delta` / `llm_call` / `coverage_gaps` / `budget_warning`）
- [x] Agent console 元件（顯示 reasoning_log 即時 stream；前端 Stage 6 步驟 28）— `agent-console-p0` 於 2026-04-29 落地（page `/explorer/[task_id]` + `ConsoleTimeline` / `StepCard` / `ProgressStrip` / `CoverageBanner` 4 元件 + `useExplorerStream` composable，bucket-fill state model，`docs/agent-explorer-spec.md §7.1`）。Apply 期同時 ingest 補裝 vitest infra（vitest + @vue/test-utils + happy-dom）；本 change 補錄了 proposal「vitest 已在 web/ 內」的事實錯誤
- [ ] Cancel 按鈕流程（前端點 → Tauri → sidecar 收 signal → 清理；後端 `run_explorer(cancel_event)` 已支援，前端未接）
...
**注解（2026-04-30，react-message-ordering-fix）**：ReAct message wire-format 鎖死為 `[system, *normalized_history, user]` — `system` 必為首位，`normalized_history` 是 `state.messages[-_MESSAGE_ROLLING_WINDOW:]` 經 `_normalize_orphan_tools` 把 orphan `tool` 訊息（state.messages 從不含 `assistant tool_calls`，所以每個 `tool` 都是 orphan）改寫成 `role="user"` 的觀察 note。違反順序會被 OpenAI Chat Completions 400（`messages with role 'tool' must be a response to a preceding message with 'tool_calls'`）。Spec 在 `openspec/specs/agent-core/spec.md` + `openspec/specs/qa-agent/spec.md` 同步。
...
**後續**
- [x] `sidecar/pyproject.toml` 用 `uv init --package` 建立（2026-04-19 m1-power-on）
- [x] `uv.lock` 進 git（2026-04-19 m1-power-on）
- [ ] CI 用 `uv sync --frozen` 確保 lockfile 一致（CI 尚未配置；現只有 pre-commit hook，正式 CI pipeline 待 Stage 7 打磨期評估）
- [x] PyInstaller 打包流程驗證（連動 D-001 後續，2026-04-19 m1-power-on）
...
- [x] qa_agent module 拆帳（`module-8-qa-p0`，2026-04-26）：`app.state.llm_qa_provider` factory 帶 `default_module="qa_agent"`，每筆 Q&A run 的 `token_usage.jsonl` line 都打 `module="qa_agent"` 讓 cost 拆帳區分 chat / generate / coverage / qa_agent 四個 chat-ish lane
...
- [x] 多檔輸出落地（`module-5-generator-p0`，2026-04-25）— `codebus_agent.generator` 套件 + 7 個 capability Requirement + ~50 unit/integration test
- [x] 採 `codebus-tutorials/` 而非 `tutorials/` root（`module-5-generator-p0` Decision 3，2026-04-25）— 改寫 `docs/module-5-generator.md` §一 / §九 對齊
...
- [x] `openspec/specs/llm-provider/spec.md`：ADDED `OpenAI embedding provider` Requirement（2026-04-23 `kb-build-production-wiring` archive 落地）
- [x] `openspec/specs/knowledge-base/spec.md`：ADDED `KB build production dependency wiring` Requirement（同 archive）
- [x] `openspec/specs/sidecar-runtime/spec.md`：ADDED `KB dependency injection hook` Requirement（同 archive）
- [x] `docs/module-2-kb-builder.md §七`：補 production wiring 段（同 archive）
- [x] `docs/llm-provider.md`：補 OpenAI embedding 段（同 archive）
- [x] `docs/sidecar-api.md §一`：`/healthz` dependency map 加 `openai_embedding` key（同 archive）
...
## D-033: Provider 三介面拆分 + Setting / Onboarding 規劃（A → B 兩個 change 順序）

**狀態**：✅ 已決方向（2026-04-28）— **Provider 抽象拆三介面（LLM / Embedding / PII）+ marker interface 雙 allowlist；分兩個 change 落地：A `split-providers-and-pii-llm` 純 sidecar 重整（Phase 6 期間可動工）/ B `provider-settings-and-onboarding` Setting Page + Onboarding wizard + Tauri keyring（blocked on Phase 6 前端收尾）**
**討論日期**：2026-04-28

### 脈絡

D-003 + llm-role-routing 期間敲定的 Provider 抽象用單一 `LLMProvider` Protocol（chat + embed union），靠 `ProviderRole`（REASONING / JUDGE / CHAT / EMBED）路由。M2 wire 完 OpenAI 後浮現三個結構性問題：

1. **單一 Protocol 變兩種實作**：`OpenAIChatProvider` 沒 `embed()`、`OpenAIEmbeddingProvider` 沒 `chat()`，union Protocol 形同虛設、`@runtime_checkable` isinstance 不可用
2. **未來要加 LLM-based PII 偵測**（Sanitizer 從純 rule-based 擴成 rule + LLM 雙模式），但 LLM PII 偵測必須看到原文，與 D-015「LLM 看到的一定是 sanitize 過的」不變式衝突 — 怎麼開洞、怎麼鎖死成爭議
3. **使用者要求多 provider + setting page 切換**：現行 ProviderRegistry 建構期 freeze、API key 走 env var、沒有 setting UI、沒有 onboarding flow — desktop app 沒設好 provider 時 sidecar 仍啟動（graceful degraded），但前端沒有引導 user 設

三件事互相牽動：要支援 multi-provider 切換 → 要 setting page → 要 onboarding flow → 要安全儲存 API key → 要 PII 模式選項 → 要 LLM PII provider 介面拆分。

### 決策（A + B 兩個 change，依序落地）

**Change A：`split-providers-and-pii-llm`（純後端，Phase 6 期間可動工）**

1. 拆三介面：`LLMProvider`（chat-shaped）/ `EmbeddingProvider`（embed-shaped）/ `PIIProvider`（detect-shaped，新介面）
2. SanitizerEngine 消費 PIIProvider — Engine 仍管 placeholder + audit，PIIProvider 只回傳 `(kind, span)` 列表（討論 Q1 選 A）
3. PIIProvider 是 marker interface — TrackedProvider 看到 inner 是 PIIProvider 就自動 bypass Pass 2，不開外部 `skip_sanitizer` flag
4. 雙 allowlist：`TrackedProvider.ALLOWED_INNER_TYPES`（chat / embed）+ `TrackedProvider.PII_ALLOWED_INNER_TYPES`（PII）；加新實作都過 `/spectra-propose`
5. PII LLM call 共用 `llm_calls.jsonl` + `token_usage.jsonl`，加 `role: "pii_detection"` + `sanitizer_pass2_applied: false` 欄位；Trust Layer UI 預設分流顯示
6. `LocalLLMPIIProvider` 建構期擋 `base_url` 必須 localhost / 內網段
7. A **不動 Registry lifecycle** — 維持建構期 freeze，mutability 留給 B

**Change B：`provider-settings-and-onboarding`（跨 Tauri + web + sidecar，blocked on Phase 6 前端收尾）**

1. Provider pool schema：`llm.providers[]` 陣列 + `llm.bindings` 把 role 對到 provider id（取代現行 `llm.roles.<role>.provider_id` 一對一綁定）
2. API key 走 Tauri keyring plugin（OS native：macOS Keychain / Windows Credential Manager / Linux Secret Service）；偏好 `tauri-plugin-keyring`（OS wrapper）而非 `tauri-plugin-stronghold`（IronOxide vault，多一道 master password 對 onboarding 不友善）
3. Tauri 啟動 sidecar 時從 keyring 讀出 keys，透過 stdin / startup IPC 注入 sidecar 記憶體（不寫磁碟、不入 audit）— 對齊 bearer token pattern
4. Setting page `/settings`：Provider 池子 CRUD + Role binding + PII 模式（rule / llm）
5. Embedding 切換 destructive — UI 獨立 confirm modal（會 rebuild KB）；其他 provider 即時、in-flight task 完整跑完、下個 task 生效
6. Onboarding wizard 三步：Welcome（1 張，僅 codebus 是什麼 + 「需要 LLM 才能跑」）→ Providers（chat × 1 + embed × 1，reasoning / judge / chat 三 role 預設共用同一個）→ Done；不允許 skip；PII 預設 rule-based 不在 onboarding 出現
7. 啟動偵測：前端打 `/healthz`，任一 dependency `not-configured` → redirect `/onboarding`
8. Registry 加 `RegistryHolder`（雙層、內層仍 immutable，外層 swap reference）或 `replace_role()` 方法達成 hot-swap

### 主要理由

1. **三介面對應三條 trust boundary**：一般 LLM 看 redacted、Embedding 看 redacted、PII 看原文 — 不同 trust 表示應該不同介面 + 不同 audit lane（落地時共用 jsonl 加欄位區分，UI 默認分流顯示）
2. **Marker + 雙 allowlist 收縮 D-015 破口**：唯一能跳過 Pass 2 的條件 = 「class 實作 PIIProvider Protocol 且進 PII allowlist」，不開外部 flag、未來不會被濫用
3. **A / B 切兩個 change**：A 純後端 + spec、易測、Phase 6 不阻擋；B 動 Tauri / web / sidecar 三層、要 design mockup、要 keyring 依賴、blocked on 前端 — 兩者性質不同，硬綁會拖慢 A
4. **Onboarding 簡短**：1 張 Welcome + 1 步 Providers + 1 張 Done。Multi-page feature tour 有反效果（user 直接點下一步沒在看），feature 介紹改 inline coach mark / dismissible tooltip 效果更好
5. **Tauri keyring 對齊 bearer pattern**：API key 跟 bearer token 一樣記憶體常駐 + 不入 audit + OS keychain 持久化，是同一條 secret-handling 不變式延伸

### 關鍵不變式（A / B land 前後都要守）

1. **API key 與 bearer 同等敏感** — 只能在 OS keychain + sidecar 記憶體存在；`llm_calls.jsonl` / 任何 audit / 錯誤訊息禁止出現原值
2. **PIIProvider 是唯一 Pass 2 例外** — TrackedProvider 對 LLMProvider / EmbeddingProvider 強制 Pass 2，不開後門
3. **PII LLM call 仍要 audit** — 共用 `llm_calls.jsonl`，靠 `role: "pii_detection"` 欄位分流；UI 默認過濾 + 提示「另有 N 筆 PII 偵測 call」，不藏匿
4. **Embedding 切換 destructive** — schema / UI / Sanitizer rules version 不要把 embedding 切換跟其他 provider 切換混為同一個 toggle，避免使用者手滑 rebuild KB
5. **Onboarding 不可 skip** — 沒設 provider 進主畫面什麼都做不了，不要假裝可以跳過
6. **A 不動 Registry lifecycle** — 維持建構期 freeze；mutability 留給 B 處理（in-flight task 行為、SSE 推送 provider id 變更通知都跟 setting UI 一起設計才合理）

### 開放問題（B 設計期要解）

- **In-flight task 切 provider 怎麼處理？** — 預設「下個 task 生效」，但 Generator / Explorer 跑到一半切 provider 會不會有 SSE 推送中斷？
- **Setting page 改了要不要重啟 sidecar？** — 大部分 binding 改動可 hot-swap registry，但 Embedding 切換綁 KB rebuild、PII LLM 模式切換要 bump rules version 重取使用者同意（`docs/authorization.md §六`）— 重啟 vs hot-swap 邊界要再敲
- **Tauri keyring plugin 跨平台 PoC** — 需 B 設計期驗證 macOS / Windows / Linux 三平台穩定度，特別是 Linux Secret Service（GNOME Keyring / KWallet）在無桌面環境下的 fallback

### B 對 Phase 6 已寫前端的影響（記下來避免未來忘記）

| 已有 / 已寫 | B 要動 |
|---|---|
| `web/app/app.vue` / `pages/index.vue`（root 進入）| 加 onboarding 偵測 redirect 邏輯 |
| `web/app/components/layout/TopBar.vue` | 加 setting 入口（齒輪 icon）|
| 全域 provider state | 新增 composable `useProviderConfig()`（或類似）|
| `web/app/components/audit/AuditPanel.vue`（O-04 LLM Call Inspector）| 顯示要兼容多 provider id + PII call 分流（`role: "pii_detection"` 過濾）— **建議在 Phase 6 實作 O-04 時就預留 prop / type，filter 邏輯可後補**|
| 沒有 | 新路由 `/settings`、`/onboarding/*` 全新元件 |

主要改動是「啟動流程 + 全域狀態」，不會回頭改 R-01 / O-04 / O-05 內容本身。**Phase 6 推進原則**：O-04 預留「filter by role」hook（type 跟 prop 加好、filter 預設 false 不擋），其他元件不需要為 B 預埋。

### 連動更新

**A 落地時（純後端）**：

- [ ] 開 `/spectra-propose split-providers-and-pii-llm` change
- [ ] `openspec/specs/llm-provider/spec.md`：拆 Protocol（LLM / Embedding 各自獨立 Requirement）、加 PIIProvider Requirement、加雙 allowlist Requirement
- [ ] `openspec/specs/sanitizer/spec.md`：Engine 消費 PIIProvider、PII LLM 的 audit role 欄位
- [ ] 新增 `openspec/specs/pii-provider/spec.md`（或併入 sanitizer）— 待 propose 時決定 capability 切法
- [ ] `docs/llm-provider.md` / `docs/sanitizer.md`：對齊新介面
- [ ] `CLAUDE.md`「三段 Sanitizer」段落補 PII LLM 例外脈絡

**B 落地時（跨 Tauri + web + sidecar）**：

- [ ] 開 `/spectra-propose provider-settings-and-onboarding` change（Phase 6 收尾後）
- [ ] `tauri/src-tauri/Cargo.toml` 加 keyring plugin 依賴
- [ ] `tauri/src-tauri/src/` 加 keyring IPC commands + 啟動時注入 sidecar
- [ ] `web/app/pages/settings.vue` + `web/app/pages/onboarding/*.vue` 全新路由
- [ ] `web/app/components/settings/*` Provider CRUD / Role binding / PII 模式 UI
- [ ] `design/v1/` 補 setting + onboarding mockup（Welcome / Providers / Done 三畫面 + Setting page）
- [ ] sidecar 加 config loader（pydantic-settings 或 tomli）+ Registry hot-swap 機制
- [ ] `openspec/specs/sidecar-runtime/spec.md` + `openspec/specs/frontend-shell/spec.md` Requirement 變動
- [ ] `docs/authorization.md §六` 補 PII LLM 模式對 rules version 的影響
...
- [x] D-033：Provider 三介面拆分 + Setting / Onboarding 規劃（A → B 兩個 change 順序）（2026-04-28）
...
- [ ] D-007：cost benchmark（實作到一定程度後；Module 4 已 land，可在 Module 5 之前安排）
- [x] D-001：PyInstaller 打包流程驗證（2026-04-19 m1-power-on 落地）
