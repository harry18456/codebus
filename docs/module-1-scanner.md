...
> **目前實作階段：`scanner-skeleton`（openspec/changes/scanner-skeleton）**
> 本 spec 描述的是 Scanner 的最終目標形狀。骨架階段只實作「檔案遍歷 +
> 類型判別 + 編碼偵測 + 語言識別 + content-type summary + 同步 `/scan` endpoint」；
> 以下子系統以 **stub 欄位 / 不動作** 形式先於 schema 預留，等後續 change 再實作：
>
> - **Sanitizer orchestration（§九）**：`FileEntry.sanitize_stats` 永遠為 `{}`；content 未經 scrub。
...
> - **Quarantine（§九、§十二）**：`stats.quarantined_count` 永遠為 `0`（沒有 sanitize timeout 路徑）。
...
### 流程
```
每個 text 檔解碼完 → ctx.sanitizer.scrub(content) → 記清理版
```
...
### 失敗處理
- 正則 timeout（Sanitizer §六）→ 檔案進檢疫區，**不進 KB**，記 `quarantined`
- Decode 成功但 sanitize 拋其他例外 → 記 error，繼續下一檔（不中止整體掃描）
...
> **Skeleton 範圍註記**：schema 欄位**一次到位**，後續 change 不得破壞相容。骨架階段
> 的 stub 預設值：
>
> - `FileEntry.sanitize_stats` → 永遠 `{}`（Sanitizer orchestration 未接）
> - `FileEntry.oversized_preview` → 永遠 `None`（§四 檔頭 200 行 summary 未實作）
> - `FileEntry.content` → 直接就是解碼後原文（**未經** Sanitizer Pass 1 scrub；Pass 2 / Pass 3 由下游負責）
> - `ScanResult.git` → 永遠 `None`
> - `ScanResult.is_monorepo=False` / `monorepo_type=None` / `sub_packages=[]`
> - `ScanStats.quarantined_count` → 永遠 `0`（沒有 sanitize timeout 路徑）
