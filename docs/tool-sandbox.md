...
**P1 symbol-navigation 工具**（`explorer-tools-p1`，步驟 19）的稽核行格式沿用上面通用 schema；`trace_import` 在掃 workspace 時若遇到指向外部的 symlink，會為每個被拒候選寫一行 `allowed=false` + `denial_reason=path_escape`（或對應分類），最後再寫一行 overall-outcome allow line（`resolved_path=<命中 path>` 或 `null`）。`find_callers` 同樣每次呼叫寫 overall allow line；額外的 Pass 1 sanitize 命中落在 `sanitize_audit.jsonl`（`pass_num=1`），跟 `read_file` 同 writer。`args_summary` 對這兩個工具故意是空 dict（`symbol` 與 `keyword` / `why` 同屬 Agent 自由文字，為避免 audit 行意外 echo 敏感字串，一律不進 whitelist）。
