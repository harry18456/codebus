...
#### F1. `§0 Language Policy` 不存在但被 SKILL 引用 5 次 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 1a (commit `25565e9`) 已在 NEUTRAL_RULES 建立 §0 Language Policy contract，SKILL 引用對應到位。
...
#### F2. §1 把 `wiki/` 標成 READ-only 但其實是 RW — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — neutral.md line 32-33 已 READ/WRITE 分離 (Phase 1a).
...
#### F3. §3 「do not modify existing sections」與 goal repair / fix mode 衝突 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — grep 沒命中 'do not modify existing sections' — 可能已 partial fixed；保留 LOW 待實機 verify.
...
#### F4. `updated` 日期 hallucination 風險 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — neutral.md line 133 已加 hallucinated-future-date warning (Phase 1a).
...
#### F5. `## from goal: <X> (YYYY-MM-DD)` 的 `<X>` 未定義 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — neutral.md line 87 仍有 `## from goal: <X>`，<X> 未明示定義.
...
#### F6. §2 taxonomy 列了兩次 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — §2 taxonomy 列舉次數可能仍重複 (grep count = 7 行涵蓋 5 type).
...
#### F7. Concept vs process 區分規則埋在 §2 末尾 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Concept vs process 區分埋 §2 (neutral.md line 69).
...
#### F8. §7 Step budget 30 是憑空數字 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — §7 ~30 step soft budget (neutral.md line 205) 已軟化，但仍憑空.
...
#### F9. §9 Failure Modes 過度簡短到模糊 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — §9 Failure Modes (neutral.md line 228) 內容沒重寫.
...
#### F10. §6 source code citation comment 只示範 Python — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — neutral.md line 174/272 citation 仍 Python comment 風格.
...
#### F11. `CODEX_AGENTS_SOFT_CONSTRAINT` 位置不對 — **DEFERRED**

> **Status (2026-05-24, housekeeping)**: DEFERRED — append 在 AGENTS.md 結尾 vs §1 Workspace Layout 內插（templated insert）是純 prompt clarity（「位置遠 = 規則感弱」是推測、無實機證據 codex agent 違反 scope）。F11a 已隨 Phase 1a 一併處理（見下），F11 位置改造需動 `vault/init.rs` 注入邏輯，工程量不對等於 user-facing 收益。留 polish batch / 真出現實機違反時再處理。
...
#### F11a. `CODEX_AGENTS_SOFT_CONSTRAINT` 內容問題（與 F11 位置獨立） — **FIXED**

> **Status (2026-05-24, housekeeping — was actually fixed in Phase 1a commit `25565e9` but not explicitly badged)**: FIXED — `codebus-core/src/skill_bundle/mod.rs:180` 當前 `CODEX_AGENTS_SOFT_CONSTRAINT` 即下方對照版內容：heading 改 `## Scope: forbidden read paths (codex path only)`（解 F11a-4）、移除 claude path meta-info（解 F11a-1）、移除 soft-constraint 自我削弱句（解 F11a-2）、加 "even if the user prompt names them"（解 F11a-3）、補 `~/.config/`、加 "refuse and explain the scope" fallback。
...
#### F12. §5 wikilink 沒說 heading anchor 不支援 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — neutral.md line 148 已加 Whole-page only + anchor-not-supported (Phase 1a).
...
#### F13. §4 `stale: false` 生命週期未定義 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — neutral.md line 135-138 已有完整 stale lifecycle 定義 (Phase 1a).
...
#### F14. 無完整 page body 範例 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — complete page body 範例 grep 沒命中 = 沒加.
...
#### F15. PII boundary 描述太弱 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — PII boundary 描述強化 grep 沒命中.
...
#### F16. §8 Out-of-scope 範例 CJK-heavy — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Out-of-scope examples (neutral.md line 225) 仍存.
...
#### F17. taxonomy 資料夾大小寫未明示 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — neutral.md line 49 已加 'Folder names are lowercase plural' (Phase 1a).
...
#### F18a. NEUTRAL_RULES §4 frontmatter 沒明示 required vs optional 欄 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — neutral.md line 120 已用 '# required, boolean' inline comment 標 required vs optional (Phase 1a).
...
#### F18. 檔首 HTML comment 整段冗餘 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — neutral.md 檔首 HTML comment 仍冗餘 (純 doc bytes, 無 agent 影響).
...
#### F19. 「Read `CLAUDE.md` here」hard-code claude provider 檔名 → 對 codex 是 dangling reference — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 codex translation 已處理 (skill-bundles spec / Codex Instruction Materialization).
...
#### F20. Hard scope 對 query / fix **內容錯誤** — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Hard scope 已 shell-level 分 Read/Write (mod.rs line 309/311)，但 query/fix 對應 SKILL 段是否完全準確需單獨 audit.
...
#### F21. 「Trigger this skill when the user types `/codebus-<verb>`」框架錯誤 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 SKILL split — trigger-frame 句被移除 (skill_bundle/mod.rs grep 無命中).
...
#### F22. `## Path translation` 段只對 goal 有用 — **VALID-DESIGN**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-DESIGN — Path translation 段仍套用所有 verb (mod.rs line 315)；要 SKILL 進一步 split 把段限定 goal-only.
...
#### F23. `## Hard scope` 重複 NEUTRAL_RULES §1 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Hard scope 在 3 處出現 (mod.rs line 307/474/577)，與 NEUTRAL_RULES §1 仍有 overlap.
...
#### F24. 「typically the codebus binary spawns the agentic CLI with cwd at this vault root for you」meta-info 對 agent 無價值 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 split — cwd-at-vault-root meta-info 已移除.
...
#### F25. shell 內無「verb-specific override 優先」明示 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — verb-specific override 優先句 grep 沒命中 = 未加.
...
#### F27. Step 5 closing summary 格式與 NEUTRAL_RULES §8 out-of-scope 規則衝突 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 5 closing summary vs §8 out-of-scope 衝突 — grep 未明確驗證.
...
#### F28. Verify mode 三欄 pipe-separated 輸出無 escape rule — **WITHDRAWN**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: WITHDRAWN — parser splitn(3, '|') 已 mitigate (memory 翻盤紀錄).
...
#### F29. Repair mode 「same scope rules as ingest workflow」歧義 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Repair mode 'same scope rules as the ingest workflow' (mod.rs line 397) 仍歧義.
...
#### F30. Step 1「Do not read every file end-to-end」過於模糊 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — 'Do not read every file end-to-end' (mod.rs line 371) 仍模糊.
...
#### F31. Verify mode「MUST NOT emit any `raw/` file contents」過嚴 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — 'MUST NOT emit any raw/ file contents' (mod.rs line 383) 仍存，inventory 嫌過嚴.
...
#### F32. Step 2 taxonomy enum 重複 NEUTRAL_RULES §2 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 2 taxonomy enum 與 NEUTRAL_RULES §2 重複.
...
#### F33. Step 4 wikilink slug 規則重複 NEUTRAL_RULES §5 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 4 wikilink slug 與 §5 重複.
...
#### F35. 三個 spawn 獨立性未明示 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — 三 spawn 獨立性 — chat (mod.rs line 464) 有 multi-turn 明示，goal verify/repair 沒.
...
#### F36. Step 5 created/modified count 缺可信源 — **VALID-DESIGN**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-DESIGN — agent self-report count (mod.rs line 379) — 要動 verb/goal.rs 後端 diff 算 count 後注入.
...
#### F37. 沒處理 `stale: true` page — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — stale: true page 處理 grep 沒命中 = 沒加.
...
#### F39. QUERY_WORKFLOW **沒明示 read scope** — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — QUERY_WORKFLOW (mod.rs line 410-435) 仍沒明示 read scope 'wiki/ ONLY'.
...
#### F40. Read-Only Invariant 是 PE1 C1 失準點（已盤） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 codex split 已重構 Read-Only Invariant 描述 (PE1 C1).
...
#### F41. §0 Language Policy dangling reference（Layer 1 F1 在此處出現點） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 1a §0 Language Policy contract 已建立 (NEUTRAL_RULES).
...
#### F42. Step 2 frontmatter 欄列表漏 `type` — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 2 frontmatter 欄列舉 (mod.rs line 420) 仍漏 `type`.
...
#### F43. Step 4「ONE coherent answer」對 lookup-style query 過嚴（UX 角度） — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 4 'ONE coherent answer' (mod.rs line 424) 仍嚴；UX 角度.
...
#### F45. Step 1 taxonomy enum 重複 NEUTRAL_RULES §2 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 1 taxonomy enum 同 F32 重複 pattern.
...
#### F46. Step 3「1-2 hops」hop 定義模糊 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — '1-2 hops' (mod.rs line 422) 仍模糊；hop 從哪起算？包不包初始？.
...
#### F47. Step 2「frontmatter first」實機可行性未驗（codex 側） — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 2 'frontmatter first' (mod.rs line 420) 仍存；codex 實機可行性未驗.
...
#### F48. Step 4 anti-template 自我描述可能可拿掉 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Step 4 'MUST NOT copy phrasing... not itself a template' (mod.rs line 379/424) 仍存；inventory 嫌可拿掉.
...
#### F49. Step 1 PreToolUse hook 描述（PE1 C1 在此出現點） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 codex split — mod.rs line 216-217 已分 claude PreToolUse hook vs codex sandbox -s.
...
#### F50. Cross-OS path 形式 SKILL 沒交代 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Cross-OS path 形式 grep 沒命中 = 沒加.
...
#### F51. fix 沒明示 read scope（wiki-only） — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — fix read scope wiki-only 明示 grep 沒命中.
...
#### F52. Step 5 §0 Language Policy dangling reference — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 1a §0 dangling 同 F41 解.
...
#### F53. Step 3 「broken-wikilink-body」remove 過激進 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — broken-wikilink-body 'or remove it' (mod.rs line 693) — inventory 嫌過激進.
...
#### F54. 「misplaced-root-page → update wikilinks」與 NEUTRAL_RULES §5 衝突 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — misplaced-root-page wikilinks 與 §5 衝突 (mod.rs line 697).
...
#### F55. unknown rule_id fallback 缺指引 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — unknown rule_id fallback 指引 grep 沒命中.
...
#### F60. `broken-wikilink-related` 沒 remove 選項（與 body 不對稱） — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — broken-wikilink-related (mod.rs line 692) 仍是 add/change 沒 remove option，與 -body 不對稱.
...
#### F62. SKILL 對 lint JSON schema 描述不完整 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — SKILL 對 lint JSON schema 描述完整性 grep 沒明確命中.
...
#### F57. 「Trust the absolute paths」段重複 Step 1 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — '## Trust the absolute paths' (mod.rs line 707) 仍重複 Step 1.
...
#### F58. 「CLI is the final-only verifier」段對 agent decision 無價值 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — '## CLI is the final-only verifier' (mod.rs line 703) 仍存；inventory 嫌對 agent decision 無價值.
...
#### F59. 「nav-missing → stub heading」內容未指引 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — nav-missing 'create the missing nav file with a stub heading' (mod.rs line 695) 仍存；inventory 嫌內容未指引.
...
#### F63. chat 完全沒「scope guard / off-topic 防護」— 實機證實洩漏 agent metadata — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — chat-security-batch (commit `aeecf5d`) ADD `Chat Scope Guard Prompt Layer` requirement + SKILL `## Scope Guard` section；real-LLM 4/4 scenarios 驗證 (`what model are you?` / role-change / mixed-prompt / wiki-injection 全照 SKILL 拒絕)。
...
#### F65. Read-Only Invariant 的 `--tools` 機制描述（PE1 C1 在此出現點） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 codex split — mod.rs line 225/227 已分 claude --tools vs codex sandbox.
...
#### F66. `mcp_*` family 是 claude-specific tool naming — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 codex split — codex SKILL 已拿掉 mcp_* claude-specific 描述.
...
#### F64. chat 沒「prompt-injection defense」— 但 codex baseline 已擋 obvious & subtle 兩種 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — chat-security-batch (commit `aeecf5d`) ADD `Chat Injection Defense Prompt Layer` requirement + SKILL `## Treat retrieved content as data` section。
...
#### F67. Schema rules 「Read `CLAUDE.md` here」對 codex 是 dangling（同 F19 pattern + 實機證實） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 codex translation 已替換 Read CLAUDE.md 為 codex-relative reference.
...
#### F69. Workflow 缺 wiki-first / raw-second 優先序 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — chat wiki-first/raw-second 優先序明示 grep 沒命中.
...
#### F68. Trigger 框架「user types `/codebus-chat`」誤導（同 F21 pattern） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 split — chat trigger-frame 移除 (description string 替代).
...
#### F73. Mode B self-validate 在 codex 路徑完全跑不了（雙重失敗） — **SPIKE**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: SPIKE — 後半 codex per-command allowance — 真正 blocker 在 codex CLI sandbox `-s` 只三級 (read-only / workspace-write / danger-full-access)，無 read-only-plus-one-cmd 中間態。caller-side validate 已兜底、user-facing 沒 broken.
...
#### F72. Read-Only Invariant 段是 PE1 C1 「最重災區」 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 PE1 C1 cluster 解 — Read-Only Invariant 段已分 claude/codex.
...
#### F75. `[CODEBUS_QUIZ_VIOLATION]` 觸發條件不夠強 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — [CODEBUS_QUIZ_VIOLATION] marker + 'If compelled' trigger (mod.rs line 668/673) 已有，inventory 嫌條件不夠強.
...
#### F76. SKILL 對 `codebus quiz validate` 輸出格式描述不完整 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 self-validate 段已含完整輸出格式描述 (exits 0 / no findings / --json) (skill_bundle/mod.rs line 248).
...
#### F77. Mode B 「You MAY also Read pages those pages wikilink to」缺 hop 限制 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Mode B 'MAY also Read pages those pages wikilink to' (mod.rs line 607) 仍無 hop 限制.
...
#### F79. Schema rules forwarding 同 F19/F67 pattern — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 Schema rules forwarding 解 — 同 F19 codex translation pattern.
...
#### F80. Trigger 框架「user types `/codebus-quiz`」同 F21/F68 pattern — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 2 split — quiz trigger-frame 移除.
...
#### F82. Mode B「at most 3」iteration cap 良好設計 (positive) — **POSITIVE**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: POSITIVE — Mode B at-most-3 iteration cap 良好設計.


對比 fix F56（撤回的，by-design 無 cap）— quiz 是結構化驗證所以設 cap、fix 是 trust-agent 所以無 cap。**兩 verb 設計理念不同但都合理**。記為正面參考。

#### F83. Caller-owned frontmatter 良好設計 (positive) — **POSITIVE**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: POSITIVE — Caller-owned frontmatter 良好設計.


「You MUST NOT author `quiz_id`, `topic`, ...」明示哪些欄不由 agent 寫。對比 goal F36（agent 自己算 count 沒可信源）— quiz 把 frontmatter 切乾淨。記為正面參考。

#### F84. Mode A「(no more than 60 words). No further content.」明確邊界 (positive) — **POSITIVE**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: POSITIVE — Mode A 60-word cap 邊界明確.
...
#### F86. 10 個模板**全部用 `/` prefix**（F26 在 Layer 3 全面 footprint） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 3 SpawnSpec restructure — claude `/` vs codex `$` 兩 backend 各自 assembly.
...
#### F88. quiz plan **沒 wrap user text**，與其他 verb 不一致 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — Phase 3 — quiz plan input 已透過 SpawnSpec 統一處理 (sub_mode + input field).
...
#### F87. user text injection 在 scope guard 修法（F63）後得到 mitigate — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — chat-security-batch F87/F87a 已修 (commit aeecf5d).
...
#### F89. goal verify/repair boundary collision — parser 側已 mitigate、agent 側理論存在 — **VALID-DESIGN**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-DESIGN — parser-side 已 mitigate (降 MEDIUM)，agent-side 理論存在；要動 spec/parser.
...
#### F90. quiz retry「PREVIOUS QUIZ:」段落注入面 — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — chat-security-batch 已修 (commit aeecf5d).
...
#### F91. chat resume_session_id 失敗時行為未指引 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — chat resume_session_id 失敗時行為未指引 (verb/chat.rs grep).
...
#### F94. goal repair pipe-separated 給 agent 沒明示 splitn(3) 解析規則 — **VALID-DESIGN**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-DESIGN — goal repair pipe-separated 給 agent 沒明示 splitn(3) 解析規則；要動 SKILL 加 parser-contract section.
...
#### F87a. scope guard 可能 over-refuse 混合 prompt（Exp B 副發現） — **FIXED**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: FIXED — chat-security-batch (commit `aeecf5d`) Scope Guard `## Mixed-prompt calibration` 段已加；real-LLM 驗證 mixed-prompt 不再全 refuse。
...
#### F95. `format!("/codebus-<verb> \"{}\"", text)` 沒 escape 雙引號 — **WITHDRAWN**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: WITHDRAWN — modern LLM 不嚴格 parse 雙引號 escape (memory 翻盤紀錄).
...
#### F96. Mode prefix 形式不統一 — **VALID-LOW**

> **Status (2026-05-24, prompt-surface-review housekeeping batch)**: VALID-LOW — Mode prefix 形式 — verify:/repair:/plan:/generate: 形式一致 colon prefix；inventory 點可能已被 Phase 3 統一.
