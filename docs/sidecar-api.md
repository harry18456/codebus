...
### task_id format
`^(scan|kb)_[0-9a-f]{8}$`。8 位 hex 由 `secrets.token_hex(4)` 產出，足夠單槽 store 不撞號。
...
- `code` 限定於 `{SCAN_FAILED, KB_EMBED_FAILED, INTERNAL_ERROR}`
- 後加入的訂閱者也會看到 cached error event（不會永遠 hang）

詳細不變式見 `openspec/changes/sse-progress-skeleton/specs/sidecar-runtime/spec.md` 的 `Background task error containment` Requirement。
