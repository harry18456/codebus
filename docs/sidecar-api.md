...
### `POST /auth/grant`
授權發放（Module O-01，`auth-flow` 2026-04-27 落地）。**sync HTTP**，**不**走 SSE / TaskRegistry / `task_id` regex（design D-A11）。

**Request body**（`GrantRequest`，Pydantic `extra="forbid"`）
```json
{
  "workspace_type": "folder",
  "workspace_source": { "path": "/abs/path/to/workspace" },
  "scenario": "first_run",
  "scope": {
...
- `workspace_type`：`Literal["folder", "topic"]`。MVP 只支援 `"folder"`；`"topic"` 通過 schema 但 handler 回 `501 AUTH_INVALID_REQUEST`（D-002 雙模 schema slot）。
- `scenario`：`Literal["first_run", "scope_reconfirm", "scope_upgrade_new_kind"]`（P0 closed set；P1 deferred 兩值見 `docs/authorization.md §五`）。
- `scope`：3 欄全 required，`outbound_endpoint` 即將送出資料的 hostname。
- `sanitizer_rules_version`：opaque 字串，handler 收到時 verbatim 寫進 audit（不做版本比對；P1 才接邏輯，見 `docs/authorization.md §六`）。
- `user_ack`：必含三條基底 flag（`raw_stays_local` / `no_kb_persist` / `outbound_to_<provider>`）+ `scope_upgrade_new_kind` 額外 `new_kind:<kind>` per acked kind。
...
```json
{
  ...
  "granted_at": "2026-04-27T10:30:00.000+00:00"
}
```

`session_id` 為 fresh UUIDv4；`workspace_id = "ws_" + sha256(canonical_lowercase_posix_path)[:12]`（同 path 跨 sidecar 重啟 stable，design D-A4）。

**錯誤回應**（HTTP 4xx / 503，`detail.code` 取自 `auth/errors.py`，**不**進 `ERROR_CODES` frozenset）
- `400 AUTH_WORKSPACE_INVALID`：`workspace_source.path` 不存在 / 不是 directory
- `400 AUTH_INVALID_REQUEST`：scenario invariant 違反（first_run 但已有 prior grant、scope_upgrade_new_kind 但無 prior、scope_upgrade_new_kind 但 new_kinds diff 為空、scope_reconfirm 但引入新 kind）
- `501 AUTH_INVALID_REQUEST`：`workspace_type="topic"` Phase 2 reserved
- `503 AUTH_NOT_CONFIGURED`：`app.state.auth_audit_logger_factory` 為 `None`（startup 失敗 / 沒注入）

**Side effects**：寫一行 `grant_issued` 到 `~/.codebus/authorization_audit.jsonl`（writer = `AuthorizationAuditLogger`，唯一 writer，design D-A2）；in-memory session 入 module-private dict（design D-A5：sidecar 重啟即失，audit log 是 canonical）。

---

### `POST /auth/deny`
使用者點 cancel（O-01 Modal）時呼叫。**sync HTTP**，回 `204 No Content`。**不**建立 session。

**Request body**（`DenyRequest`，Pydantic `extra="forbid"`）
```json
{
  "workspace_type": "folder",
  "workspace_source": { "path": "/abs/path/to/workspace" },
...
- `reason`：`Literal["user_cancelled", "app_closed"]`（P0 closed set；`dialog_dismissed` 留 P1）

**Response 204** No Content。

**錯誤回應**：`501 AUTH_INVALID_REQUEST`（topic mode）/ `503 AUTH_NOT_CONFIGURED`。

**Side effects**：寫一行 `grant_denied` 到 `~/.codebus/authorization_audit.jsonl`；session dict 不變（design D-A9）。

---

### `POST /auth/revoke`
撤回授權。**sync HTTP**，回 `204 No Content`。clear session + 寫 audit。

**Request body**（`RevokeRequest`，Pydantic `extra="forbid"`）
```json
{
  "session_id": "01928b1f-7c3a-4f9e-9c8b-a1b2c3d4e5f6",
...
- `trigger`：`Literal["settings_revoke"]`（P0 closed set；`rules_version_bump` / `provider_change` / `workspace_deleted` 留 P1）

**Response 204** No Content。

**錯誤回應**
- `404 AUTH_NO_ACTIVE_GRANT`：`session_id` 在 in-memory dict 中找不到（含 sidecar 重啟後再 revoke 的情況）
- `503 AUTH_NOT_CONFIGURED`

**Side effects**：寫一行 `grant_revoked` 到 audit log（含 `grant_ts` 從 in-memory session 帶出）；從 in-memory dict 移除 `session_id`（後續同 id revoke 會 404）。

---

### `GET /auth/status`
查當前 workspace 的授權狀態。**sync HTTP**。

**Query params**
- `workspace_id`（required）：由前端先呼 `workspace_id_for_path()` 或從 prior `GrantResponse` 帶過來
...
```json
{
  ...
  "session_id": "01928b1f-7c3a-4f9e-9c8b-a1b2c3d4e5f6",
  "last_grant": {
    "ts": "2026-04-27T10:30:00.000+00:00",
    "event": "grant_issued",
    "session_id": "01928b1f-...",
    "workspace_id": "ws_a3f2b1c8d4e5",
    "scenario": "first_run",
    "scope": { "llm_provider": "anthropic", "...": "..." },
    "sanitizer_rules_version": "2026-04-20-1",
    "user_ack": ["raw_stays_local", "no_kb_persist", "outbound_to_anthropic"]
  },
  "current_rules_version": "2026-04-20-1"
}
```

- `has_active_grant`：`true` 僅當 audit log 最新 `grant_issued.session_id` 仍在 in-memory dict 中（sidecar 重啟即翻 `false` — design R-3 trade-off）
- `session_id`：當 `has_active_grant=true` 時帶出對應 session id，否則 `null`
- `last_grant`：最新一筆 `grant_issued` 完整 audit row（從 `~/.codebus/authorization_audit.jsonl` 線性掃出），workspace 從未授權則 `null`
- `current_rules_version`：handler 取請求時當下 `codebus_agent.sanitizer.RULES_VERSION` 值，前端 P0 不需用（P1 比對邏輯才用）

**錯誤回應**：`503 AUTH_NOT_CONFIGURED`。**不會**有 404 — workspace 未授權回 `last_grant: null`，不視為錯誤（design R-3）。

**Side effects**：無；每次 call 線性掃 audit log（design D-A6：P0 不做 in-memory cache）。
...
#### Auth HTTP error codes（NOT in ERROR_CODES frozenset）

四個 sync HTTP error code 從 `sidecar/src/codebus_agent/auth/errors.py` 取，**故意**與上方 SSE wire-error frozenset 不交集（design D-A11）。auth endpoints 走同步 HTTP，不流經 SSE 通道；混進 frozenset 會破壞「`ERROR_CODES` 是 SSE 通道封閉集合」的不變式。

| Code | 觸發場景 | HTTP 狀態 |
|---|---|---|
| `AUTH_WORKSPACE_INVALID` | `POST /auth/grant` 收到的 `workspace_source.path` 不存在 / 不是 directory | 400 |
| `AUTH_INVALID_REQUEST` | `scenario` invariant 違反（first_run 已有 prior、scope_upgrade 無 prior、scope_upgrade 無 diff、scope_reconfirm 引入新 kind）；或 `workspace_type="topic"`（501） | 400 / 501 |
| `AUTH_NO_ACTIVE_GRANT` | `POST /auth/revoke` 對 in-memory dict 找不到 `session_id` | 404 |
| `AUTH_NOT_CONFIGURED` | `app.state.auth_audit_logger_factory` 為 `None`（startup 失敗 / 沒注入） | 503 |

新增 auth code 必須走新 Spectra change 同步擴 `auth/errors.py` + 對應 spec scenario，**不可**加進 `tasks.py::ERROR_CODES`。
