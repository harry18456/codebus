...
> ⚠️ **已封存／部分過時**（停在 2026-05-20）。此後 codex + Azure provider 已 ship（§6 out-of-scope 的「多 AI provider」與 §7 的 multi-provider idea 皆已落地），多項 Stage 進度亦已推進。當前真實狀態以 `openspec/specs/` + `docs/internal/BACKLOG.md` 為準；本檔保留作為 pivot 與規劃史。
>
> 起草：2026-05-08（path D pivot 起點）。最後更新：2026-05-20。
>
> 涵蓋 CLI + app 兩條主線（歷史記錄）。
...
- **`verify-stage-independent-model`** — macOS / Linux GUI deferred；Tauri app smoke（cargo tauri dev → Settings 開啟 EndpointSection 看 4-row 含 verify、改 verify model 後 Save、reload 後新值留存）deferred 至 polish-ship。Windows 端 backend + frontend contract 已驗證 pass：codebus-core endpoint 12 條 parse / resolve 新測試 + STARTER_CONFIG round-trip + verb/quiz + verb/goal verify spawn model routing CLI integration tests + EndpointSection 4 個新 verify-row vitest + SettingsModal round-trip + `codebus init` 寫 starter config 含 verify 區塊 smoke + 刪 verify 後 `codebus query` 觸發 fail-loud parse error 訊息含明確 field path 「claude_code.system.verify: required when active=system」smoke。既有 user 升級指引在 `docs/2026-05-21-verify-stage-independent-model-migration.md`。
- **`pretooluse-image-block-toggle`** — macOS / Linux GUI deferred；Tauri app smoke（cargo tauri dev → Settings 看「Block image / binary reads」toggle 渲染、預設 ON、切 OFF 後 Save、reload 留存）deferred 至 polish-ship。Windows 端 backend + frontend contract 已驗證 pass：codebus-core config::hooks 9 條單元測試 + STARTER_CONFIG round-trip + hook.rs config-gate 單元測試 + hook_check_read.rs 7 條新 integration tests（disabled allows / missing config fail-safe / absent section fail-safe / malformed yaml fail-safe）+ app-tauri hooks namespace round-trip + store readHooksConfig 6 條 + SettingsModal 4 條 toggle vitest + CLI smoke 四連（init 寫 hooks section / default block / false→allow / 刪 section fail-safe block）。`hooks.read_image_block` 預設 true，既有 user yaml 無 hooks section → tolerant default true → 行為不變、**無 migration doc 需求**（跟 verify-stage 的 required fail-loud 故意相反）。
- **`wiki-open-in-obsidian`** — GUI 手動 smoke 全部 deferred（agent 無法驅動互動式桌面 GUI）：Windows 端 `cargo tauri dev` →（a）開已 register Obsidian 的 vault → Wiki tab 選一頁 → 看到 `[Open in Obsidian]` → 點 → Obsidian 跳對頁；（b）開沒 register 的 vault → 按鈕不出現 —— 待 **user 實機驗收**；macOS / Linux GUI 一併 deferred 至 polish-ship。Windows 端 backend + frontend automated contract 已驗證 pass：app-tauri `ipc::wiki` 9 條新單元測試（build_obsidian_url spec 四列 encoding + 不在 wiki_root 回 None；get_obsidian_vault_id Some / None / no-config-dir / parse-failure→AppError；open_wiki_in_obsidian valid-slug URL / unregistered→Invalid(obsidian) / unknown-slug→Invalid(slug)）+ IPC registry count 27→29（mod.rs 兩條 count/names 測試 + keyring_ipc integration count 測試同步更新）+ codebus-core `lookup_vault_id_at` 改 pub（CLI 既有 consumer，app 加可注入測試）+ 前端 wiki store 4 條 obsidianVaultId 測試（probe 回 id / 回 null / error fail-soft 仍列頁 / reset 清空）+ WikiPreview 4 條按鈕測試（content 兩鈕並存 / nav 僅 Obsidian / vaultId null 不渲染 / 點擊 invoke open_wiki_in_obsidian 一次帶 slug）+ ipc.ts wrapper tsc pass + i18n key。設計用 vault id（非 name）對齊 CLI lint OSC 8 既有 `obsidian://open?vault=<id>&file=<rel>`；URL 組裝 + 相對路徑 + percent-encode 集中在 Rust 純函數。

- **`agent-backend-seam`** — GUI 手動 smoke deferred（agent 無法驅動互動式桌面 GUI）：`cargo tauri dev` → Settings → Endpoint/AI Provider 區 →（a）改一個 model / effort（如 goal=opus-4-7）→ Save enable → 存 →（b）關 app 確認 `~/.codebus/config.yaml` 寫成新 **`agent: providers: claude: ...`**（非舊 `claude_code:`）→（c）從 GUI 跑一個 goal/query 確認用到新設定 —— 待 **user 實機驗收**；macOS / Linux 一併 deferred 至 polish-ship。Windows 端 backend + CLI + frontend automated + 真實 binary smoke 已驗證 pass：codebus-core 497 lib + 整合測試（既有 agent-spawn / verb 行為測試 assertion 不變即過＝零行為改變鐵證）+ config schema 統一 `agent.providers.claude` 解析/驗證測試 + STARTER_CONFIG round-trip + app-tauri 144（含 save 驗證 reject/accept azure 改 agent 形狀）+ 前端 vitest 429（SettingsModal fixtures 遷 agent schema）+ tsc。**真實 binary 實跑全 CLI 指令** pass：init 寫新 schema、query(ReadOnly+haiku) / goal(Workspace+opus-4-6) / quiz(plan→generate, generate 帶 `Bash(codebus quiz validate *)`) / fix(Workspace+`Bash(codebus lint *)`) / chat(ReadOnly) 的 argv 全正確且 byte-equivalent（唯一刻意差異＝quiz-generate 移除冗餘裸 Bash）；手寫 GUI 形狀 `agent.providers.claude` 非預設 model config → CLI 用 `claude-opus-4-7`（證明 app↔CLI 接通）。**待補的純前端遷移已在本 change 一併完成**（settings store / ipc.ts / ipc/config.rs validate 改讀 `agent`），故 GUI 存檔已是 CLI 認得的 schema。
