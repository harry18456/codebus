...
| 2026-06-28 | codebus MCP Server **v3.1.0**：multi-vault（`codebus mcp` 無 `--vault` → registry 模式服務全部已登錄 vault、`--vault` 維持 v1 pinned；省略 vault 時 `wiki_list`/`wiki_search` 跨全部 present vault 聚合並標來源、`wiki_read` 需明確 vault）+ 一鍵接入 claude/codex（Settings 兩列獨立開關、shell out `claude mcp add --scope user` / `codex mcp add`、寫 bundle CLI 絕對路徑 + dev fallback、不碰 client config）。app-state 讀取下放 `codebus-core::app_state`（移動非複製、`read_app_state` 唯讀不建檔）；vault 白名單 canonicalize、`raw/code` 不可達。實機驗真 claude+codex add/list/remove + ✔Connected + GUI toggle 往返。IPC 29→32 — 原開放 CAP-3 延伸（v2 好用度/agentic query 見 CAP-7） | `mcp-multi-vault-and-client-install`（feat `2ea338f` + 版號 `2efaf50` + archive 本批；spec mcp-server MODIFIED + mcp-client-install ADDED + app-shell MODIFIED） | 詳細 [mcp-server](2026-05-14-mcp-server-backlog.md) + CAP-7（v2 follow-up）|
