...
**Init UX 升級（manual test 2026-05-04 期間 user 提案分三層）:**

Phase 1 init 是 silent — `codebus --repo X` 直接動 disk（建 `.codebus/`、改 source `.gitignore`、init nested git）沒 confirm 也沒 explain。對 power user OK，對首次使用者突兀。Phase 1 已落地 Layer 1，Layer 2 / 3 留 phase 2/3：

- ✅ **Layer 1 — claude CLI prerequisite check**（phase 1 已落地，commit pending）：跑 init/goal/query 之前先 `claude --version`，沒裝 fail-fast + install hint。`src/infra/cli-detect.ts` + `src/cli.ts` wire 在 vault sanity check 之後
- **Layer 2 — Per-vault init confirmation**（phase 1.5 / 2）：互動模式下 `codebus --repo X`（init only）先列出將要做的事（建 .codebus/ / 改 .gitignore / nested git init）+ `(y/N)` confirm。需要 TTY 偵測 + `--yes` flag 跳過 + readline 或 prompt library。CI / 自動化用 `--yes` 維持原行為
- **Layer 3 — Global setup wizard**（phase 2/3，跟 multi-provider 綁）：`codebus setup` subcommand 一次性 global 設定。跟 multi-provider 整合（問 default model）、跟 token tracking 整合（問 budget cap）、跟 emoji preference 整合（寫 `~/.codebus/config.yaml`）。第一次跑 codebus 時若 config 不存在且 stdin 是 TTY，可建議 `codebus setup`
