...
CLI 主線（`docs/v3-roadmap.md`）2026-05-10 全 ship 後，app 層 v1 切成 4 條序列化 change。每一條都假設前一條已 archive；不是平行可換序。
...
| 3 | `v3-app-quiz-cmdk` | Quiz flow（pending / reviewing 兩態 + md 持久化） + Cmd+K spotlight 查詢（streaming + 引用） | workspace-goal 的 wiki page model |
| 4 | `v3-app-polish-ship` | Release build / installer / auto-update / icon 視覺再優化 / E2E test infra / **跨平台驗證（含 v3-app-foundation / workspace-goal / quiz-cmdk 各自 acceptance checklist 在 macOS / Linux 重跑）** | 前三條都 ship |
...
7 週工作量。單一巨大 change 的歷史教訓：apply 失焦、review 不可行、in-flight spec drift。本 roadmap 的切點來自 2026-05-11 brainstorming session — 每一條落點都是「換到下一條時，前一條跑得起來的 demo」（不是「實作了某個檔案」），所以 archive 任一條後都可以對外展示一個可用的 app 子集。

## Out of scope（全部 v1 範圍以外）

下列 item 在 v1 四條 change **皆不做**，未來走獨立 change 評估：
