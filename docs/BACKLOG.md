...
| 2026-05-28 | Bash check-bash hook 缺 subprocess 層 integration test（Read hook 有 `hook_check_read.rs`、Bash hook 無；6 個 spec scenario 只 by-proxy 驗 bool predicate、decision-JSON 契約 + fail-closed 分支無端對端 test） | 測試完整度（無 live bypass、verification gap） | 輕（建 `hook_check_bash.rs` 鏡像 read hook test，半天） | [cli-quality-review F4](2026-05-22-cli-quality-review.md)（test 完整度段） || 2026-05-28 | Claude-trace 分析 long propose prompt 的 token / cache / context 用量（每 change 200+ 行 prompt × 多 session 累積成本未量化） | workflow efficiency / 複利成本 | 半天 | [claude-trace-prompt-analysis](2026-05-28-claude-trace-prompt-analysis-todo.md) |
