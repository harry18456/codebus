...
| 2026-05-30 | per-run wall-clock timeout（`invoke()` 無計時器、hang / 無人值守會無限期占住；重用現成 `KillHandle.terminate_tree()` + 新增 `interrupt_reason=Timeout`、limit 走 config 預設不限） | robustness / circuit breaker（provider-neutral） | 輕（小） | [cli-applicability T1-1](2026-05-30-cli-capability-applicability-backlog.md) |
| 2026-05-30 | codex 內層 `is_error` 餵進 outcome（codex 頂層 exit 0 遮蔽 sandbox denial、被擋下的 run 誤標 succeeded；訊號已由 `codex_parser` 解出卻只進渲染） | 正確性 / 可觀測性 | 中 | [cli-applicability T1-2](2026-05-30-cli-capability-applicability-backlog.md) |
| 2026-05-30 | token 累加器對 codex 改 cumulative-replace（codex usage 非 per-turn delta、一次 spawn >1 `turn.completed` 會 double-count；先在 0.135.0 重驗） | multi-provider parity（潛伏 double-count） | 輕（小） | [cli-applicability T1-3](2026-05-30-cli-capability-applicability-backlog.md) |
| 2026-05-30 | claude 非 chat verb 加 `--no-session-persistence`（codex 已 `--ephemeral`、claude 漏 → orphan session 無界成長 + prompt/wiki 隱私足跡） | hardening / 隱私 + 磁碟 | 輕（一行 conditional） | [cli-applicability T1-4](2026-05-30-cli-capability-applicability-backlog.md) |
| 2026-05-30 | Rust loader 補 effort enum 校驗（GUI Save 才閘 5-level enum、CLI / 手改 yaml 漏；只對 claude，model 名維持寬鬆） | hardening（fail-late） | 輕（小） | [cli-applicability T1-5](2026-05-30-cli-capability-applicability-backlog.md) |
| 2026-05-30 | skill bundle drift 守門（codex body 由 `claude_to_codex_translate` 6× `str.replace` 衍生、改 claude 段落會讓 codex body 靜默留過時 claude 文字含假機制描述、無 test 守） | 正確性（靜默餵錯 provider 指令） | 中 | [cli-applicability T2-1](2026-05-30-cli-capability-applicability-backlog.md) |
