...
codebus 不含 LLM — 先裝好一個 agent CLI 並登入：

- [Claude Code](https://claude.ai/code)（預設，OAuth 登入），或
- [OpenAI Codex](https://github.com/openai/codex)（含 Azure OpenAI 部署，見 [Provider 選擇](#provider-選擇)）

從原始碼裝 CLI 另需 Rust 1.85+（edition 2024）。

> **💸 花費提醒——有訂閱不代表免費。** codebus 是用 headless 模式（`claude -p`）驅動 claude。自 2026-06-15 起，Claude 訂閱方案（Pro / Max / Team / Enterprise）下這類用量改從每月獨立的 **Agent SDK credit** 扣（如 Pro 為每月 US$20），不算在互動用量額度內——credit 用完後 Agent SDK 請求會停止直到下個帳期重置（若有啟用 usage credits 則改以標準 API 費率計費）。API key / Azure 部署照 token 計費；codex provider 同樣會消耗你的 OpenAI / Azure 額度。詳見 [Use the Claude Agent SDK with your Claude plan](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan)。
...
Windows 另有預建安裝檔（桌面 app + 內嵌 CLI），放在 [GitHub Releases](https://github.com/harry18456/codebus/releases) — 見[桌面 app](#桌面-app)。
...
- **Codex**：裝 [`codex` CLI](https://github.com/openai/codex)。provider 切換、model / effort、Azure base_url / api_version 都在[桌面 app](#桌面-app) 的 Settings → Endpoint 設定（CLI 目前不提供 provider 切換指令）。Azure API key 放進 OS keyring，或設環境變數 `CODEBUS_AZURE_KEY=<your-key>`（keyring 取不到時的通用 fallback，claude / codex 皆適用）。

provider 設定全在 app 的 Settings 介面設；CLI 共用同份 `~/.codebus/config.yaml`。
...
`goal` / `query` / `chat` / `quiz` 裡你打的字會變成 system prompt 的一部份送進 agent。
...
需要 Rust 1.85+（edition 2024）與至少一個 provider CLI（見[前置需求](#前置需求)）。Windows / macOS / Linux 都能 build；主力 dev 在 Windows MSVC。
...
找早期乘客中 — issue / PR / 報 bug / 提建議都歡迎（見 [CONTRIBUTING.md](CONTRIBUTING.md)），駕駛座還有空位。
