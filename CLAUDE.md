...
M1「power-on」通電（2026-04-19 archive）後，資料層 + Module 4 Explorer Agent（步驟 16-23 P0+P1+coverage+context+golden 全 archive）已補齊；**Module 5 Generator P0**（`module-5-generator-p0`，2026-04-25 archive）通電多檔輸出；**Module 8 Q&A P0**（`module-8-qa-p0`，2026-04-26 archive）通電 RAG-first 兩階段 + 第七層 `kb_growth.jsonl` audit + `POST /qa` endpoint。Stage 5 Backend 全部收尾，Review #2 7 條 Critical 也走 `review-2-critical-fix`（2026-04-26 archive）清掉。下一階段：**Phase 6 前端**（`docs/implementation-plan.md` §二第六階段，先 `auth-flow` 落 R-01 / O-04 / O-05 / O-01 四站 mockup）。
...
| 2026-04-25 | `spec-cleanup-stage-4-batch-1` | Stage 4 review #1 Cat 2 收尾：4 個 capability spec 真錯修 + 8 個 spec @trace 清理 + 1 處 production code dead write 移除。`usage-tracking` `Sanitizer-ready field reserved` Scenario 移除 M1 過時 wording（production 永遠 true）；`explorer-sse` `Explorer loop emits agent_thought / agent_action_result / judge_verdict events` 主文加 P0 placeholder 句 + 新 Scenario `tokens_used field accepts P0 placeholder zero` 合法化 `tokens_used=0`；`agent-core` 兩條 MODIFIED：`ReAct loop ...` Requirement 加 Scenario `Update step uses tool_name as P0 pending_queue placeholder`（合法化 `_update_state` push tool_name 到 pending_queue 的 placeholder 行為，明寫真符號等 explorer-tools-p2）/ `ReasoningLogger appends one JSONL line per Step ...` 加 Scenario `Logger is the single source of truth for prompt version stamping`（明寫 `model_copy` 覆寫是強制行為，callers MUST NOT pre-stamp）；`sidecar-runtime` `Background task error containment` 主文加 `/explore` 並顯式列出錯誤碼表（`SCAN_FAILED` / `KB_BUILD_FAILED` / `EXPLORE_FAILED`）+ 新 Scenario `Explore task exception surfaces as safe error event`；production code `sidecar/src/codebus_agent/agent/explorer.py` coverage round Step 移 `explorer_prompt_version` / `judge_prompt_version` 兩個 dead write（已被 `ReasoningLogger.write` `model_copy` 覆寫）+ 對應兩個孤兒 import；apply 階段 grep 全 `openspec/specs/` 發現 17 處 M1 boilerplate `@trace code: web/dist`（agent spot-check 漏掃 13 處），分布 8 個 spec（`llm-provider` / `qdrant-client` / `app-packaging` / `repo-layout` / `tauri-shell` / `tool-sandbox` / `usage-tracking` / `sidecar-runtime`）全部清光，`@trace` block 保 source/updated 兩欄；零 LLM 行為改變、零 schema 變動、零 fixture re-baseline；`uv run pytest sidecar/tests/` 698 passed / 19 skipped 與 baseline 一致 |
| 2026-04-25 | `audit-path-unification` | Cat 2.5-B 收尾：把 5 層 workspace audit JSONL 全統一到 `<ws>/.codebus/` 子目錄（既有 `sanitize_audit.jsonl` / `tool_audit.jsonl` 已在新位置；本 change 把 `token_usage.jsonl` / `llm_calls.jsonl` / `reasoning_log.jsonl` 從 `<ws>/` root 搬到 `.codebus/`）。新 leaf module `sidecar/src/codebus_agent/api/_audit_paths.py` 收 6 個常數（`_WORKSPACE_AUDIT_SUBDIR=".codebus"` + 5 個 `_<NAME>_FILENAME`），avoid `api/__init__.py` ↔ `api/explore.py` circular import；`api/__init__.py` 三個 factory（`_make_tracker_factory` / `_make_provider_factory` / `_make_chat_provider_factory`）共 5 處 path string 改用常數 + `.codebus/` 子目錄；`api/explore.py` import 常數、ReasoningLogger 構造前 caller-side `mkdir(parents=True, exist_ok=True)` 確保 `.codebus/` 存在（spec 規定 ReasoningLogger 不 auto-mkdir、UsageTracker / LLMCallLogger 已 auto-mkdir）；`usage-tracking` 兩條 MODIFIED Requirement（`UsageTracker writes token_usage.jsonl` + `LLMCallLogger writes llm_calls.jsonl` 全部 path mention 改 `.codebus/`）+ `agent-core` 一條 MODIFIED Requirement（`ReasoningLogger appends one JSONL line per Step ...` path 改 `.codebus/` + 加 caller-mkdir 約束 + `Path stays under workspace and caller mkdirs .codebus parent` Scenario rename + 補述）；新 defensive test `test_factory_audit_paths_land_under_codebus_subdir`：assert 7 個 factory（`kb_provider` / `kb_query_provider` / `kb_usage_tracker` / `llm_reasoning_provider` / `llm_judge_provider` / `llm_chat_provider` / `llm_coverage_provider`）都寫入 `<ws>/.codebus/<file>.jsonl`，drift guard 鎖死 convention；`uv run pytest sidecar/tests/` 699 passed / 19 skipped（baseline 698 + 新 defensive test 1，零 regression）；Stage 6 R-01 panel 一個 `<ws>/.codebus/*.jsonl` glob 涵蓋六層 workspace audit；Module 5 Generator 動工自然繼承新 path 零回頭；CLAUDE.md 七層段位址全更新 + 拿掉 latent risk 段；Cat 2.5-B 全部解掉 |
| 2026-04-25 | `module-5-generator-p0` | Module 5 Generator P0 通電（步驟 24）：新 `codebus-agent.generator` 套件 9 個 module（`types` / `runner` / `station` / `validator` / `stable_id` / `frontmatter` / `moc` / `route` / `log` + `prompts/`）+ 7 個新 capability Requirement + 2 個 sidecar-runtime MODIFIED Requirement（`task_id format` 擴 `^(scan\|kb\|explore\|generate)_[0-9a-f]{8}$`、`Background task error containment` 加 `GENERATE_FAILED`）；新 `POST /generate` endpoint + `app.state.llm_generate_provider` factory（`role=ProviderRole.CHAT, default_module="generate", temperature=0.4`）；新 audit path 常數 `_GENERATOR_LOG_FILENAME = "generator_log.jsonl"`（第六個 workspace-level filename，落 `<ws>/.codebus/`）；4 條 Decision 落地 — Decision 1（output 過 Pass 1 Sanitizer，`source.pass="generator"`）/ Decision 2（`Station.depends_on` backfill 留 follow-up，`route.json` `prerequisites=[]` P0）/ Decision 3（root 改 `<ws>/codebus-tutorials/{task_id}/` 而非 `tutorials/`）/ Decision 4（per-station retry quota 3 + degraded stub 寫入後其他站不受影響、disk write 失敗不重試）；prompt date-version `STATION_PROMPT_VERSION="2026-04-25-1"`；測試 49 generator unit/integration + 4 endpoint = 53 新測；docs 連動更新 `docs/module-5-generator.md` §一 / §九、`docs/decisions.md` D-029 後續清單兩 `[x]`、`docs/implementation-plan.md` 步驟 24 改 🚧、`docs/reviews/2026-04-25-stage-4.md` Module 5 預警 5 項全 `[x]` |
| 2026-04-25 | `review-backlog-cleanup` | Stage 4 review #1 Cat 3 backlog 收尾兩條：**(A) `rules_version` 集中常數**（不變式 9 收緊）`sanitizer/config.py::_BUILTIN_RULES_VERSION` 改名 `RULES_VERSION` 公開常數 + 保留 `_BUILTIN_RULES_VERSION = RULES_VERSION` backward-compat alias；`sanitizer/__init__.py` re-export；`api/__init__.py:81` + `api/scan.py:55` 兩處字面量 `"2026-04-20-1"` 改 `from codebus_agent.sanitizer import RULES_VERSION as _RULES_VERSION`（callsite `_RULES_VERSION` 私名不變）；新 defensive test `tests/sanitizer/test_rules_version_constant.py` 用 `is` identity check 鎖死 — 三處 callsite 必同物件，drift 即時測爆；spec MODIFIED `sanitizer` capability `Rules version is recorded on every audit line` 主文加單一常數約束 + 新 Scenario `Single source of truth for rules_version constant`。**(B) chat `cost_usd` 算真值**（D-021 / D-022 後續清單）新 module `providers/pricing.py::estimate_chat_cost_usd(model, prompt_tokens, completion_tokens) -> float` + `_CHAT_PRICING: dict[str, tuple[float, float]]`（`gpt-4o-mini-chat-v1`: (0.15, 0.60) / `mock-chat-v1`: (0.0, 0.0) placeholder）；未知 model 回 `0.0` + 走 `module-level logger.warning(...)` 標出未知 model id；zero-token 不噴 warning（healthz / 空 prompt 不污染 log）；`tracked.py::chat()` 兩處 `cost_usd=0.0` 改成 `cost_usd_for_chat = estimate_chat_cost_usd(model_id, prompt_tokens, completion_tokens)` 算一次餵 `_tracker.record(...)` + `_emit_usage_delta(...)` — `token_usage.jsonl` 與 `usage_delta` SSE event 拿同個 float（audit 與 wire 一致）；spec MODIFIED `usage-tracking` capability `UsageTracker writes token_usage.jsonl` 主文加 pricing-table 行為 + 兩個 Scenario（`Known chat model writes non-zero cost_usd` / `Unknown chat model logs warning and writes zero cost_usd`）；新 5 測（`tests/sanitizer/test_rules_version_constant.py` 1 + `tests/providers/test_pricing.py` 3 + `tests/providers/test_tracked_chat_cost.py` 2）；既有 28 個 providers + agent + api 測 0 regression（`mock-chat-v1` 占位價 (0.0, 0.0) 不噴 warning）；既有 28 個 golden 測 0 regression（`cost_usd` 無硬斷言）；`docs/reviews/2026-04-25-stage-4.md` Cat 3 #1 + #4 兩條 `[x]`、`docs/decisions.md` D-021 + D-022 後續清單各補一條 `[x]` |
| 2026-04-26 | `module-8-qa-p0` | Module 8 Q&A P0 通電（步驟 25）：新 `codebus_agent.agent.qa` 套件（5 budget 常數 `_QA_MAX_STEPS=10` / `_QA_MAX_ADD_TO_KB_PER_SESSION=20` / `_QA_MAX_CHUNK_SIZE_CHARS=2000` / `_QA_MAX_ADD_TO_KB_PER_QUESTION=5` / `_QA_DEDUP_THRESHOLD=0.95` + `_hits_confident` 三條件 top-1>0.75 / top-3 mean>0.65 / top-5 entity coverage + `run_qa` 三階段 RAG-first → 可選 ReAct → synthesize）+ 新 `agent.prompts.qa`（`QA_SYSTEM` 三段「值得沉澱」+ `render_qa_prompt` + `QA_PROMPT_VERSION="2026-04-26-1"`，隔離 explorer/judge/coverage prompts）+ 新 `agent.tools.qa_tools.QATools` 七工具集合（5 reused read tools delegate FolderTools + `kb_search` + `add_to_kb`）+ 新 `agent.tools.kb_search` / `add_to_kb`（後者五階段 pipeline budget→sanitize→validate→upsert→growth-log）+ `agent.types` 加 `QAState` / `QAAction` / `QAAnswer` / `KBCitation` 四個 Pydantic + `Step.qa_prompt_version` 欄；新 `kb.growth_logger.KBGrowthLogger` 唯一 writer（auto-mkdir + station_id pre-validation + `event_type="add"` hardcode 不暴露 kwarg）+ `_KB_GROWTH_FILENAME="kb_growth.jsonl"` 第七 audit filename；`KnowledgeBase.query` 加 keyword-only `filter_stations` + `upsert_chunk` 雙層 dedup（`exists_by_hash` → embed → `find_similar(0.95)`，回 `"dedup:hash"` / `"dedup:sim"` / point_id）；`agent.reasoning_logger` 加 `mode="qa"` 模式排除 explorer/judge versions 寫 `qa_prompt_version`；新 `POST /qa` endpoint + `app.state.kb_growth_logger_factory` + `app.state.llm_qa_provider` factory（`module="qa_agent"` 拆帳）+ `task_id` regex 擴 `^(scan\|kb\|explore\|generate\|qa)_[0-9a-f]{8}$` + `QA_FAILED` 錯誤碼 + 503 `QA_NOT_CONFIGURED`（5 個 dependency slot 必齊）；新 SSE event `rag_hits`（1 次，初始 KB 探查後）+ `kb_growth`（per new chunk，dedup-skip 不 emit）+ `qa_answer`（terminal）；2 ADDED capability spec（`qa-agent` 8 Requirement、`kb-growth` 4 Requirement）+ 3 MODIFIED（`knowledge-base.query` filter_stations + `upsert_chunk`、`sanitizer` Pass 3 hook、`sidecar-runtime` task_id+QA_FAILED+POST /qa）；測試 ~67 新測（5 audit_paths + 6 growth_logger + 3 query_filter + 4 upsert_chunk + 4 pass3_audit + 4 qa_prompts + 3 qa_types + 5 kb_search + 8 add_to_kb + 3 qa_tools + 1 budget + 4 hits_confident + 5 run_qa + 7 qa_endpoint + 5 qa_sse + 2 e2e_integration），全 suite 823 passed / 19 skipped；docs 連動：`docs/implementation-plan.md` 步驟 25 改 🚧、`docs/decisions.md` D-016 / D-022 後續清單各補一條 `[x]`、`docs/sidecar-api.md` POST /qa schema 補完 |
| 2026-04-26 | `review-2-critical-fix` | Stage 5 Review #2（`docs/reviews/2026-04-26-stage-5.md`）7 條 Critical 一次性收尾：**CR-1** `api/tasks.py::ERROR_CODES` 字面量 `KB_EMBED_FAILED` → `KB_BUILD_FAILED`（無 alias）+ `_classify_exception` / `_safe_error_message` 同名 rename + 兩條 defensive guard test（`test_error_codes_frozenset_exact_ten_elements` 鎖死 10 元素精確等於、`test_kb_embed_failed_alias_not_present` 防 alias 復活）。**CR-2** `agent/tools/add_to_kb.py:124-131` 三層 fallback chain（`getattr(sanitizer, "rules_version", None) or "rules-unknown"` + `try / except: pass` import）整段拆掉，改檔頂 module-level `from codebus_agent.sanitizer import RULES_VERSION` 直接使用（對齊 `folder_tools.py` / `tracked.py` 既有模式）；`test_rules_version_constant.py` 加兩條 defensive guard（`test_add_to_kb_uses_rules_version_constant_directly` identity check、`test_no_rules_unknown_literal_in_add_to_kb` source-level grep）。**CR-3** `kb/knowledge_base.py::upsert_chunk` 簽名 `str` → `tuple[str, str]`（`outcome ∈ {"new", "dedup_hash", "dedup_sim"}` + 真實 `point_id`）+ 新 private helper `_lookup_existing_point_id_by_hash`（走 `backend.search_points(query_filter={"text_hash": ...})` 拿 hash dedup 命中的真實 point id，未來可換成 Protocol method）；`add_to_kb.py:181-194` 解構 tuple → `growth_logger.write(point_id=real_point_id, dedup_skipped=outcome.startswith("dedup_"))`；4 個既有 Scenario test 改寫成 tuple-based assertion + 新 `test_dedup_hash_returns_real_existing_point_id`（嚴格 equality）+ 新 integration `test_dedup_path_writes_real_point_id_to_kb_growth`（assert `<ws>/.codebus/kb_growth.jsonl.entry_id` 為 UUID 格式）。**CR-4** `docs/sidecar-api.md` §四 + `docs/qa-agent.md` §八訊息流圖兩處 `answer_stream` → `qa_answer`（`{type, answer, citations: [{file_path, line_start, line_end, related_stations}]}` 一次性 non-streaming，P1 reserved 註明）。**CR-5** `docs/sidecar-api.md` §三 POST /generate 段重寫對齊 `GenerateRequest` Pydantic `{workspace_root, task, stations, options}` + `extra="forbid"` + Response 202 + 錯誤碼表（`GENERATE_NOT_CONFIGURED` / `GENERATE_WORKSPACE_INVALID` / `TASK_IN_FLIGHT` / `GENERATE_FAILED`）。**CR-6** `docs/sidecar-api.md` §四拿掉 `usage_summary` event 整段 + `done` 事件改述「不需任何前置 event；client-side 由累積 `usage_delta` 算 session total」+ `docs/agent-core.md` `emit_summary` 註記退役 + `docs/decisions.md` D-021 連動條目同步。**CR-7** `docs/sidecar-api.md` §三-bis `task_id` regex `^(scan\|kb)_` → `^(scan\|kb\|explore\|generate\|qa)_` + ERROR_CODES 表補齊 10 個 + `docs/module-2-kb-builder.md` 連帶 `KB_EMBED_FAILED` rename。新 integration test `test_pass3_sanitize_audit_records_real_rules_version` 用真實 `SanitizerEngine` + `SanitizerAuditLogger` assert `<ws>/.codebus/sanitize_audit.jsonl` 第一行 `rules_version == "2026-04-20-1"` 真值；3 MODIFIED capability spec（`sidecar-runtime` `Background task error containment` 加 alias guard + `Error code table is exhaustively enumerated` Scenario / `knowledge-base` `upsert_chunk` 簽名改 tuple + 4 Scenario 全改寫 / `kb-growth` `Required fields ...` 主文加「real Qdrant point id」+ 新 Scenario `Dedup-skipped write records existing point id` & `New write records new point id`）；全 suite 830 passed / 19 skipped（baseline 823 → +7 新測）；docs 連動：`docs/reviews/2026-04-26-stage-5.md` Critical 段 7 條全 `[x]` + 標 archive 日期 |
| 2026-04-27 | `audit-path-unification-stage-2` | Stage 5 Review #2 Cat 2.5 4 條 cross-cutting code drift 一次性收緊：**(1) `*.jsonl` 字面量收回 `_audit_paths.py`** — `api/scan.py:48-49` 拆掉重複定義 + `api/qa.py:207` / `agent/tools/folder_tools.py:118,473` 三處字面量改 import；補新 module-level grep test `tests/test_no_jsonl_literal_drift.py`（`['"][\w_-]+\.jsonl['"]` 整 `sidecar/src/codebus_agent/` 只能命中 `_audit_paths.py`）+ 第二測 `test_seven_audit_filenames_present_in_canonical` 鎖死 7 個 filename 都在 canonical leaf；`sanitizer` capability `SanitizerAuditLogger appends each replacement to JSONL` 主文加單一常數約束 + 新 Scenario `Filename literal is single-sourced in canonical leaf module`（推廣 `kb-growth` 既有的 `kb_growth.jsonl` single-source 紀律到全 7 層）。**(2) `_STATION_ID_RE` 6 處 → single source**（apply 階段重新 grep 從 5 處擴成 6 處，補加 `kb/payload.py:23`）— 新 leaf module `sidecar/src/codebus_agent/agent/station_id.py`（types-only，只 `import re`）匯出 `STATION_ID_RE: re.Pattern` + `_STATION_ID_RE` backward-compat alias + `validate_station_id(sid)` raise `ValueError` + `find_invalid_station_id(ids)` 回第一個違反 id；6 處 callsite（`agent/tools/add_to_kb.py` / `agent/tools/kb_search.py` / `kb/growth_logger.py` / `kb/knowledge_base.py` / `kb/payload.py` / `api/qa.py`）改 `from codebus_agent.agent.station_id import _STATION_ID_RE`；4 處 ValueError message 內 raw-string `r"^s\d{2}-..."` 字面量改用 `f"{_STATION_ID_RE.pattern}"` 動態取（徹底落實 single-source）；`api/qa.py:51` 從 string `_STATION_ID_RE = r"..."` + `re.fullmatch(_STATION_ID_RE, v)` 一併改 method form `_STATION_ID_RE.fullmatch(v)`；新 `tests/agent/test_station_id_constant.py` 三條 defensive test：`is`-identity 鎖 6 callsite + `re.compile(r"^s\d{2}-...")` 字面量整 source-tree 0 命中（含 raw-string 變體）+ helpers raise / return 行為。**(3) `_QA_MAX_*` budget 常數 single source** — `agent/tools/add_to_kb.py:37-39` 拆掉 3 個重複定義改 `from codebus_agent.agent.qa import _QA_MAX_CHUNK_SIZE_CHARS, _QA_MAX_ADD_TO_KB_PER_SESSION, _QA_MAX_ADD_TO_KB_PER_QUESTION`；新 `tests/agent/test_qa_constants_single_source.py` 三條 defensive：`is`-identity 鎖 add_to_kb 3 個常數 + `^_QA_(MAX|DEDUP)_*` line-anchored 定義整 `sidecar/src/codebus_agent/` 只能在 `agent/qa.py` + dedup threshold 同 source。**(4) `_QA_DEDUP_THRESHOLD` single source** — `kb/knowledge_base.py:51` 拆掉重複 `_QA_DEDUP_THRESHOLD: float = 0.95` 改 `from codebus_agent.agent.qa import _QA_DEDUP_THRESHOLD`；同步從 `kb.knowledge_base.__all__` 移除（避免重 export 給下游再走 KB 路徑拿）；4 個 MODIFIED capability spec 各加 Scenario `Station id regex sourced from canonical leaf module` / `Dedup threshold sourced from canonical single source` / `All callsites import constants from agent.qa single source` / `Filename literal is single-sourced in canonical leaf module`；全 suite 843 passed / 19 skipped（baseline 830 → +13 新測：3 station_id + 3 qa constants + 2 jsonl drift + 5 由 spec scenario 觸發既存 path）；`docs/reviews/2026-04-26-stage-5.md` Cat 2.5 段 4 sub-cat + 進度狀態表全 `[x]` + 標 archive 日期 |

**目前沒有 in-progress change**。下一步：**Phase 6 前端**（`docs/implementation-plan.md` §二第六階段 ~8d）。動工順序：
- **步驟 26**：`@nuxtjs/mdc` + Checkpoint.vue / Quiz.vue / QAEntry.vue 元件（2d，依賴步驟 24）
- **步驟 26.5 `auth-flow`**：O-01 Grant Modal + `authorization_audit.jsonl` writer + 4 sidecar endpoints（4d，建立 `openspec/specs/authorization-audit/` capability，吃掉 `docs/authorization.md` 410 行設計，**Trust Layer Act 1 第一幕**）
- **步驟 27**：R-01 站牌列表 + 內容區 + progress.json 讀寫 + 解鎖邏輯（2d）
- **步驟 28**：R-01 內 Agent console（reasoning_log SSE stream，Demo 神器，1.5d）
- **步驟 28.5**：O-04 LLM Calls 分頁（list + detail modal + filter，1d）+ O-05 Sanitizer Diff（在 R-01 內，配套 audit unlock endpoints）
- **步驟 29**：三個介入點（路線調整 / 重跑 / 換資料夾，1.5d，依賴步驟 23 + 27）
- **步驟 30**：Q&A 聊天 UI + 引用 panel + KB growth 稽核 tab（1d，依賴步驟 25 + 28）
...
- Module 5 輸出多檔（D-029，`module-5-generator-p0` Decision 3 改 root 為 `codebus-tutorials/`）：`<ws>/codebus-tutorials/{task_id}/tutorial.md`（MOC 索引）+ `stations/s{NN}-slug.md`（每站一檔，含 YAML frontmatter schema_version=1 + stable station id；跨檔用標準 markdown link，禁 wikilinks）+ `route.json`（D-029 §八 schema）。Operational log `<ws>/.codebus/generator_log.jsonl` 是 per-Module 例外（與 `reasoning_log.jsonl` 同層）
...
**七層 Audit JSONL**（workspace-level 六層全在 `<ws>/.codebus/` + App-level 一層在 `~/.codebus/`；implementation status 真實標注）：

實作狀態：✅ 已實作 / ⏳ 待對應 Module 落地 / 📐 design-only（spec 在 docs/，capability spec + writer 待對應 change）

- ✅ `<ws>/.codebus/sanitize_audit.jsonl`（Sanitizer 命中；Pass 1 Scanner + Pass 2 Provider 都會寫，帶 `pass_num` 欄）
- ✅ `<ws>/.codebus/tool_audit.jsonl`（Sandbox 工具呼叫；Module 4 Explorer 實作時填入）
- ✅ `<ws>/.codebus/kb_growth.jsonl`（Q&A `add_to_kb`；`module-8-qa-p0` 落地的第七層 workspace audit）：唯一 writer 是 `KBGrowthLogger`，`event_type` P0 永遠 `"add"`（`"rollback"` 預留 P1，writer 簽名不暴露 kwarg）；station_id pre-validation + ISO 8601 UTC `ts` + 12 必填欄位（`ts` / `session_id` / `question` / `originating_station_id` / `entry_id` / `source` / `related_stations` / `reason` / `sanitize_stats` / `chunk_size_chars` / `dedup_skipped` / `event_type`）；factory `app.state.kb_growth_logger_factory` 是 workspace-scoped、無 OpenAI key 時為 `None`
- ✅ `<ws>/.codebus/reasoning_log.jsonl`（ReAct 每 step；Module 4 實作時填入；`audit-path-unification` 從 `<ws>/` root 搬到 `.codebus/` 子目錄）
- ✅ `<ws>/.codebus/token_usage.jsonl`（D-021 + `usage-tracker-dedup`）：唯一寫入路徑是 `TrackedProvider`，`module` 欄由構造時的 `default_module` 寫入（目前 8 個 lane：`kb_build` / `kb_query` / `reasoning` / `judge` / `chat` / `coverage` / `generate` / `qa_agent`）；`audit-path-unification` 從 `<ws>/` root 搬到 `.codebus/`
- ✅ `<ws>/.codebus/llm_calls.jsonl`（D-022）：完整 wire payload，`sanitizer_pass2_applied` 在 sanitizer-safety-chain 後真會翻 true；`audit-path-unification` 從 `<ws>/` root 搬到 `.codebus/`
...
**Path constants** 集中在 `sidecar/src/codebus_agent/_audit_paths.py`（package-root leaf module，避免 `api/__init__` ↔ `api/generate` ↔ `generator/runner` 三角 circular import；`api/_audit_paths.py` 是 backward-compat re-export shim）：`_WORKSPACE_AUDIT_SUBDIR=".codebus"` + 七個 `_<NAME>_FILENAME` 常數（`_SANITIZE_AUDIT_FILENAME` / `_TOOL_AUDIT_FILENAME` / `_TOKEN_USAGE_FILENAME` / `_LLM_CALLS_FILENAME` / `_REASONING_LOG_FILENAME` / `_GENERATOR_LOG_FILENAME` / `_KB_GROWTH_FILENAME`）。`UsageTracker` / `LLMCallLogger` / `KBGrowthLogger` constructor auto-mkdir parent；`ReasoningLogger` 不 auto-mkdir（spec 規定 caller-side path safety），caller (`api/explore.py` / `api/qa.py`) 自己 mkdir。

**同 single-source pattern 的姊妹 leaf module**：`sidecar/src/codebus_agent/agent/station_id.py`（`audit-path-unification-stage-2` 落地）匯出 `STATION_ID_RE: re.Pattern` + `_STATION_ID_RE` backward-compat alias + `validate_station_id` / `find_invalid_station_id` helpers — 6 處 callsite（`agent/tools/add_to_kb.py` / `agent/tools/kb_search.py` / `kb/growth_logger.py` / `kb/knowledge_base.py` / `kb/payload.py` / `api/qa.py`）走 `from codebus_agent.agent.station_id import _STATION_ID_RE`，`tests/agent/test_station_id_constant.py` 用 `is`-identity + source-grep 鎖死跨層不可分歧。`agent/qa.py` 同樣是 5 個 `_QA_(MAX|DEDUP)_*` 常數的單一 owner（`agent/tools/add_to_kb.py` + `kb/knowledge_base.py` 反向 import 進來），`tests/agent/test_qa_constants_single_source.py` 配 `tests/test_no_jsonl_literal_drift.py` 兩條 source-grep test 一起把整 package 的 `*.jsonl` 字面量、`_QA_*` 定義、station id regex 全鎖在各自 canonical leaf。

**使用者 .gitignore**：加 `.codebus/` 一行即可排除全部 workspace audit。
...
cd sidecar
uv sync                            # 裝依賴 + 建 venv
uv run pytest                      # 全測（~830 passed + ~19 skipped；Qdrant / symlink 相關環境相依會自動 skip）
