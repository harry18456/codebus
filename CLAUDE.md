...
M1「power-on」通電（2026-04-19 archive）後，資料層 + Module 4 Explorer Agent（步驟 16-23 P0+P1+coverage+context+golden 全 archive）已補齊；**Module 5 Generator P0**（`module-5-generator-p0`，2026-04-25 archive）也通電完成（`<ws>/codebus-tutorials/{task_id}/` 多檔輸出 + Pass 1 over output + degraded fallback + per-station retry quota=3 + `POST /generate` endpoint）。下一個里程碑是 **Module 8 Q&A P0**（`docs/implementation-plan.md` 步驟 25，D-016 / `kb_growth.jsonl` writer + Pass 3 sanitizer hook）。
...
**目前沒有 in-progress change**。下一步：**步驟 26** Phase 6 前端（`auth-flow` / R-01 Workspace + O-04 LLM Call Inspector + O-05 Sanitizer Diff + O-01 Grant Modal）。
...
- ✅ `<ws>/.codebus/token_usage.jsonl`（D-021 + `usage-tracker-dedup`）：唯一寫入路徑是 `TrackedProvider`，`module` 欄由構造時的 `default_module` 寫入（目前值：`kb_build` / `kb_query` / `reasoning` / `judge` / `chat` / `coverage`）；`audit-path-unification` 從 `<ws>/` root 搬到 `.codebus/`
...
**Path constants** 集中在 `sidecar/src/codebus_agent/api/_audit_paths.py`（leaf module 避免 circular import）：`_WORKSPACE_AUDIT_SUBDIR=".codebus"` + 五個 `_<NAME>_FILENAME` 常數。`UsageTracker` / `LLMCallLogger` constructor auto-mkdir parent；`ReasoningLogger` 不 auto-mkdir（spec 規定 caller-side path safety），caller (`api/explore.py`) 自己 mkdir。
