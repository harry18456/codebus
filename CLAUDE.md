...
**目前沒有 in-progress change**。下一步依 `docs/implementation-plan.md`：**步驟 20 `coverage-gap-recurse`**（翻開 `_COVERAGE_RECURSION_ENABLED` 並填 recursion body），或走 **步驟 21 `context 壓縮 + token-aware budget`**（Explorer 大型 repo 效能問題）。
