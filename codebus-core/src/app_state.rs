//! Read/write helpers for `~/.codebus/app-state.json` — the codebus
//! **app-state registry** of known vaults.
//!
//! This file-persistent state was originally defined in the desktop app
//! (`codebus-app`), but it is pure `fs` + `serde` with no Tauri dependency, so
//! it lives in `codebus-core` and is shared by two callers:
//!
//! - the **app**, which is the sole *writer* (add/remove vault) — it re-exports
//!   these symbols from its thin `state::app_state` wrapper, which additionally
//!   owns the non-persistent `AppRuntimeState` Tauri runtime state.
//! - the **CLI `mcp` subcommand**, which *reads* this file (read-only) as the
//!   multi-vault registry when started without `--vault` (see the `mcp-server`
//!   capability and `codebus-cli/src/mcp/registry.rs`).
//!
//! File schema (`AppState`):
//! - `{ "schema_version": 1, "vault_list": [...] }`
//! - Missing file → write empty state and return it.
//! - Parse failure OR `schema_version > CURRENT` → log warn to stderr, return
//!   an in-memory empty state, and DO NOT touch the file on disk.
//! - `CODEBUS_HOME` env var redirects the home root for tests / containers
//!   (matches the `config::default_config_path` convention so the CLI and app
//!   agree on home resolution).
...
pub fn load_app_state(path: &Path) -> AppState {
    match fs::read_to_string(path) {
        ...
        Err(err) => {
            eprintln!("warning: app-state.json read failed (using empty list): {err}");
            AppState::empty()
        }
    }
}

/// Read-only load: NEVER creates, modifies, or deletes the file. A missing,
/// unreadable, malformed, or future-schema file all yield an in-memory
/// `AppState::empty()`. Used by the CLI `mcp` subcommand's registry reader,
/// which must not write `app-state.json` (writes are the app's job). Unlike
/// [`load_app_state`], a missing file does NOT get created.
pub fn read_app_state(path: &Path) -> AppState {
    match fs::read_to_string(path) {
        Ok(text) => parse_state(&text),
...
/// Parse app-state JSON text, downgrading a parse error or an
/// unsupported-future `schema_version` to an empty in-memory state (with a
/// stderr warning). Shared by [`load_app_state`] and [`read_app_state`].
...
mod tests {
    ...
    #[test]
    fn read_app_state_missing_file_is_empty_and_creates_nothing() {
        let tmp = TempDir::new().unwrap();
        let path = fixture_path(&tmp);
        assert!(!path.exists());
        ...
        assert!(
            !path.exists(),
            "read_app_state MUST NOT create the file (registry is read-only to the CLI)"
        );
    }

    #[test]
    fn read_app_state_reads_existing_without_rewrite() {
        let tmp = TempDir::new().unwrap();
        let path = fixture_path(&tmp);
        let state = AppState {
            schema_version: CURRENT_SCHEMA_VERSION,
        ...
        let loaded = read_app_state(&path);

        assert_eq!(loaded, state);
        assert_eq!(fs::read_to_string(&path).unwrap(), before, "read must not rewrite");
    }
