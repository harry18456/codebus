//! No-op scanner. The 0.2.0 behavior-preserving default for `raw_sync`.
...
mod tests {
    ...
    fn null_scanner_returns_empty_even_when_input_contains_secret_lookalike() {
        // Defensive contract pin: even if the input contains text that
        // OTHER scanners would flag (an AWS access key here), the null
        // scanner MUST stay silent. Otherwise default behavior diverges
        // from 0.2.0 and the "preserved behavior" guarantee breaks.
        let s = NullScanner::new();
