...
## Design v1.1 Reply Status（2026-05-26）

Design v1.1 mocks 交付。Source：`v1.1-mocks.html`（5 個 design 灰區 view 正式化）。

**Phase 6 全部 unblock**——LoadingOverlay / 02c Interrupted / Quiz wizard / Wiki page reader / ChatWidget 5 個 view spec 都 lock 到「能直接開工」細度。

**5 條 ⚠ 對齊點決議**（2026-05-26 全採我建議）：

| # | 議題 | 決議 |
|---|---|---|
| 1 | LoadingOverlay 階段 1/6 太快可能閃過 | **保 6 階段、加 minimum 300ms per phase**（同階段 5 Obsidian skip strategy）；實機跑 1 週若仍視覺閃再收 |
| 2 | Failed 「看錯誤詳情」 inline 還是 modal | **inline expand**——`LocalizedError` 已是 structured object、inline 塞容易；保留 audit trail 上下文 |
| 3 | Quiz Step 1 範例 pill 用 wiki page metadata 能拿到嗎 | **可行 ✅**——`WikiTree.tsx` 已 access wiki store + frontmatter `title`、不用 fetch full content；vault 空時 fallback 5-bucket 各 1 hard-coded |
| 4 | Wiki frontmatter 有 `last_updated_by` (goal ID) + `last_updated_at` 嗎 | **已 verify `frontmatter.rs:44-55`**：`goals: [...]`（authoring goals 列表）+ `updated`（timestamp）+ `sources[]`（count）全在。Metadata bar 取 `goals[last]` 作 latest authoring goal、**無 backend 改動** |
| 5 | ChatWidget modal mode 共用 floating chat session | **延續、不 reset**——「同 widget 不同呈現」 mental model 一致；spotlight「乾淨輸入框」期待靠 modal 預設 focus input + 只顯示最新 N 條 history 平衡 |
...
**~~等 v1.1 handoff~~** → **v1.1 已交付**（2026-05-26）：5 個 design 灰區 view（LoadingOverlay / 02c Interrupted / Quiz wizard / Wiki page reader / ChatWidget）全 spec lock。詳見文首 `## Design v1.1 Reply Status` + 對應 sections。
...
- **6 phase 規格（design v1.1 lock 2026-05-26）**：

| # | zh-tw | en | InitEvents 對應 |
|---|---|---|---|
| 1 | 準備車庫 | Preparing garage | `Start` · `LayoutCreated` · `SourceGitignore` |
| 2 | 複製源碼並掃過敏感資料 | Copying source · scrubbing secrets | `PiiConfigLoaded` · `PiiWarn` · `RawSyncDone` |
| 3 | 建立獨立 git 倉庫 | Setting up isolated git | `InternalGitignoreDone` · `NestedRepoDone` |
| 4 | 搭起 wiki 結構 | Building wiki structure | `SchemaDone` · `ManifestDone` · `NavStubsDone` · `SkillBundlesDone` · `SettingsDone` |
| 5 | 註冊到 Obsidian | Registering with Obsidian | `ObsidianResult`（可 skip） |
| 6 | 上路前最後檢查 | Final checks | `StarterConfigDone` · `CommitDone` · `Finished` |

- **Layout**：72px 🚌（`codebus-bus-roll` 動畫 ±22px / ±1° / 1.8s loop、bus 動畫**不重置**——ambient 訊號跨階段穩定）+ 靜態標題「公車正在發車…」+ 階段副標（隨 phase 變）+ 6-dot 階段指示器（沿用 Quiz wizard step dots 同支元件）
- **階段語意規範**：
  - 進入新階段：副標更新 + dot 點亮（bus 不重置）
  - 同階段內多 InitEvent **不重複切換副標**——一個階段渲染一次副標、停留到下個階段
  - 階段 5 Obsidian skipped → dot 直接 done；強制 **minimum 300ms 停留**（避免 user 一閃即逝感困惑）
  - **對齊點 1 決議**：**所有階段都加 minimum 300ms per phase**（同 strategy）、實機跑 1 週若 1/6 仍閃才收進相鄰階段
  - Finished → overlay **fade-out 200ms** 收掉（不要 instant 消失）
- **異常處理**：
  - 單一階段超過 **20s** 沒進下一階段 → 副標下方加 dim hint「（這步比平常久一點，再等等…）」
  - InitEvent emit error → 切「失敗模式」：bus 停下、step dot 變紅、標題改「車子卡住了」、副標寫 `LocalizedError`、加 retry 按鈕（amber-warm 視覺、跟 02c Interrupted 同色語、不 hard-fail red）
  - 無 backend 進度（rollout 早期 fallback）→ UI 維持 v1 靜態副標但開啟 bus 動畫、不 break
- **要做的事**：
  - Tauri 層：`add_vault_at` 改 async + accept AppHandle、把 InitEvent → Tauri event `vault-init-progress` emit 給 frontend
  - Frontend：LoadingOverlay listen 該 event、6 phase state machine、minimum 300ms timer
  - i18n：新增 `loading.phase.{1..6}.title` zh + en key
...
##### R2 · Empty state 缺 content header row [local] [archived 2026-05-27]
...
##### R3 · Empty state 視覺設計（design 灰區，走 Y 方向）[local] [archived 2026-05-27]
...
##### R4 · Empty state 英文 hard-code [i18n Cat B] [archived 2026-05-27]
...
##### R5 · Content 垂直幾何置中、大留白 [shared G1] [archived 2026-05-27]
- 走 R3 三段式 layout 後自動緩解；無需單獨修

##### R6 · `+ New goal` CTA 位置 [local] [archived 2026-05-27]
...
| Mode | 用途 | 規格 |
|---|---|---|
| **1. Collapsed bubble** | idle 待命 | fixed bottom-right 16px / 16px、44×44 圓鈕、`bg-raised` + 1px `border-strong`、20px 💬 emoji；hover → 邊框 `accent-ring` + scale(1.04)；click → 開 floating |
| **2. Floating panel** | inline 沿襲、不擋畫面 | **360×460 固定大小（拿掉 resize handle）**；bottom-right anchored；header (💬 + title 「Ask about this vault」+ 右側 `⤢` open in modal / `▿` minimize)；body = ChatTranscript；input bar；Esc / 點外面**不關閉**（黏著） |
| **3. Centered modal** | spotlight 體驗（取代 cut 掉的 05 Cmd+K Overlay）| **640 wide × max 480 tall**、top 60px from viewport (不正中、視覺重心偏上)；backdrop blur 2px + 55% black；header 含 `⤡` dock to floating + `✕` close；Esc / 點 backdrop 關閉 |

**Mode 切換矩陣（v1.1 lock 2026-05-26）**：

| 從 | 動作 | 到 |
|---|---|---|
| bubble idle | click | floating |
| bubble idle | `⌘K` | modal |
| floating | `▿` minimize | bubble idle |
| floating | `⤢` expand | modal |
| floating | `⌘K` | modal · 關 modal 後**回 floating** |
| modal | Esc / click backdrop | 回到觸發前 mode（bubble 或 floating）|
| modal | `⤡` dock | floating |
| modal | `✕` close | bubble idle |

**Chat session 共享規則**（**對齊點 5 決議**）：
- **3 mode 共用同一 chat session**——「同 widget 不同呈現」 mental model；session reset 等於兩個 widget、增加複雜度
- chat session 跟著 vault 走（per-vault chat history、不跨 vault）
- Mode 偏好**不持久化**——每次 vault open 都從 bubble idle 開始；⌘K 是 mode 切換 fast-path
- Modal 「乾淨輸入框」期待靠：modal input field 預設 focus + 只顯示最新 N 條 history scroll 平衡（**不 reset session**）

**Citation 規範**：
- ChatWidget bot reply 內 wiki page reference **全用 `.cite-link`**（dashed-amber-mono）——chat 內全是 citation 性質、不是 navigation；同 WP11 規格

**Loading / Streaming / Error state**：
- **Loading**：bot 回應途中加 spinner row「（spinner） 想中…」、user message 下方、不另開 message bubble
- **Streaming**：bot message 邊收邊 render、結尾 blinking amber caret（沿用 02 stream tail caret）
- **Error**：bot reply 渲染為 **amber-tint** error message（不 red）——ChatWidget 錯誤幾乎都 retryable（network drop / token limit / context too long），跟 Interrupted goal 同色語

**Promote-to-goal**：
- bot reply 後面有「↗ promote to goal」 chip、dashed border、hover 時 amber tint
- Click → 開 NewGoalModal + prefill 對話的 user message 當 goal；chat **stays in ChatWidget**（不消失）

**i18n Cat C 全解**：
- aria-label keys：`chat.open` / `chat.minimize` / `chat.expand-to-modal` / `chat.dock` / `chat.close`

### Goals Populated Gap

- **現況截圖**：`codebus-app/scripts/.goals-list.png`（2026-05-25，3 個 goal row：1 done + 2 interrupted）

##### GP1 · 缺 content header row [shared with R2] [archived 2026-05-27]
...
##### GP2 · `+ New goal` 位置錯 [shared with R6] [archived 2026-05-27]
- 現況在右上 topbar；應在 content header row 右側
- 跟 R6 一起修

##### GP3 · 缺 `RECENT` section label + count [shared with G4 / S5] [archived 2026-05-27]
...
##### I1 · Notice banner 三變體 [i18n Cat B] [design v1.1 spec lock]
- **現況**：「App was closed before this goal finished. Wiki state may be partial — review in terminal if needed.」（純英文 + terminal jargon）
- **新規格**（design v1.1 lock 2026-05-26）：banner 視覺都一樣（amber-tint banner + mono heading label），只內文不同。Backend 帶 `interrupt_reason` 欄位：

| reason | heading label | 內文 |
|---|---|---|
| `"app-close"` | `interrupt · app-close` | 中途離開了 codebus，wiki 還沒寫完。可以重跑這個 goal 補上。 |
| `"user-cancel"` | `interrupt · user-cancel` | 你取消了這個 goal。已寫的 wiki 內容會保留，重跑會從頭開始。 |
| `"network-drop"` | `interrupt · connection-lost` | codebus 中途失聯了。可以重跑這個 goal 試試看。 |
| `"unknown"` (default) | `interrupt` | 這個 goal 沒跑完。可以重跑、或留著看記錄到哪。 |

- **Heading label 用 `.mono` 風格**——半技術詞、metadata 性質，不是寫給每個 user 看的核心訊息；內文才是 user-facing message
- **i18n**：4 條 `goalDetail.interrupt.{reason}.heading` + 4 條 `goalDetail.interrupt.{reason}.body` zh + en key

##### I-failed-vs-interrupted · 視覺差表 [design v1.1 lock]

為避免 Interrupted（amber）跟 Failed（red）混淆，視覺差不只 color：

| | Interrupted | Failed |
|---|---|---|
| Status pill | amber dot + `Interrupted` | red dot + `Failed` |
| Header action | amber primary `↻ Retry` | secondary 「看錯誤詳情」（不主動鼓勵 retry）|
| Banner 語氣 | 「可以重跑」「沒寫完」— 行動性 | 「codebus 撞到問題」+ error message — 解釋性 |
| Goals list dot | amber 7px dot | red 7px dot |

**對齊點 2 決議**：Failed 「看錯誤詳情」用 **inline expand**——保留 audit trail 上下文；`LocalizedError` 已 structured object，inline 容易塞
...
##### I-state-machine · 02c State machine [design v1.1 lock]

| 事件 | 動作 |
|---|---|
| goal 跑中 · app 收到 close 訊號 | 標記 `interrupt_reason: "app-close"` · 寫入 GoalRunStore · 不嘗試 graceful shutdown LLM call（接受 partial state） |
| goal 跑中 · user 按 Cancel | 標記 `interrupt_reason: "user-cancel"` · 終止 codex sandbox · 保留已寫 wiki |
| goal 跑中 · network drop / sandbox crash | 標記 `interrupt_reason: "network-drop"` · 寫入錯誤 cause 到 stream log（不顯示在 banner、只在展開的 stream log 裡） |
| 進入 02c view | 讀 reason 渲染對應 banner · stream log default closed |
| 按 Retry | 建立新 GoalRun · 原 interrupted run **保留在 list**（不刪、有 audit value）· retry 結果是獨立 row |
| 原 interrupted run 在 list 上 | amber dot · title 同原本 · **不主動標** 「retried at 14m ago」（避免列表雜訊；想知道就點進來看）|

**Backend contract**：goal run 結束時 emit 帶 `interrupt_reason` 的 event；GoalRunStore 持久化此欄位
...
- **資料來源**（已 verify `codebus-core/src/wiki/frontmatter.rs`）：
  - `<goal>` = frontmatter `goals[last]`（latest authoring goal）；frontmatter schema 已有 `goals: [...]`，**無 backend 改動**
  - `<time-ago>` = frontmatter `updated`、relative format（走 `common.minutesAgo` 等 i18n keys）
  - `<N> sources` = body 內 wikilink count（不含 goal name 那個）；**< 1 時整段不顯示**
- **互動**：
...
- **完整文案**：「想改這頁？跑一個 goal 跟 codebus 說該怎麼改 →」
- **互動**：點「跑一個 goal」 → 開 NewGoalModal、**自動 prefill**：
  - `goal: "修改 wiki/<page-path>.md — <描述你想改的地方>"`
...
- Hint card 規格：
  - 36px 📂 emoji `fg-quaternary` 色
  - 大字 16px `fg-secondary`：「選一頁開始讀。」
  - 小字 12px `fg-tertiary`：「或點下方旅行日誌看 codebus 跑過什麼。」
...
```css
/* 普通 wikilink：wiki body 內連結到別頁。Navigation 屬性。 */
.wiki-body a.plain-wikilink {
  color: var(--fg);
  text-decoration: underline;
  text-decoration-color: var(--border-strong);
  text-underline-offset: 3px;
}
.wiki-body a.plain-wikilink:hover {
  color: var(--accent);
  text-decoration-color: var(--accent);
}

/* Citation wikilink：03b quiz citation block 內的引用。Provenance 屬性。 */
.citation-block a.cite-link {
  font-family: 'JetBrains Mono', monospace;
  color: var(--accent);
  text-decoration: underline;
...
- ChatWidget bot reply 內 wiki page reference **全用 `.cite-link`**（chat 內全是 citation 性質、不是 navigation）
...
### Quiz wizard view 統一規格（design v1.1 spec lock）

- 點 `+ New quiz` → topbar `+ New quiz` **hidden 不是 disabled**
- header 變「New quiz」 + step indicator dots + label
- content area 是 wizard step body
- 每步可 Cancel / Back
- **退出回 Quiz history**（不退到 Lobby / Goals empty，continuity 重要）
- **4 個 step（design v1 lock）**：`topic → scope → generating → answering`（+ completion 收尾，技術上 step 4 後的狀態）
- **answering step**：step-dot 不換，但 header right 改顯示 `Q3 / 5`（per-question 是 live concern）

### Quiz wizard Step 1 · Topic [design v1.1 spec lock]

- **Layout**：wizard header (back / title "New quiz" / step dots / step label "Step 1 / 4 · Topic" / Cancel) + 大字標題「想驗證哪個主題？」+ 副標「寫下你想被 quiz 的範圍 — codebus 會挑相關 wiki 頁面出題。」+ 文字 input + **範例 pill 區** + hint 「點擊範例直接填入。⏎ 送出。」+ footer Cancel + amber primary 「下一步 →」
- **input placeholder**：「例如：auth 怎麼運作 / IM 適配器系統 / 對話傳遞流程」
- **範例 pill 來源（design v1.1 ack + 對齊點 3 已驗）**：用 **vault 既有 wiki page 標題**（不 hard-coded）——`WikiTree.tsx` 已 access wiki store + frontmatter `title`，不用 fetch full content
  - 最多 3-5 個；vault page 多時隨機 sample
  - Vault 完全空時 fallback：5-bucket 各 1（hard-coded 5 條範例）
- **Empty input + 按下一步**：不前進、input 加 amber 邊框 + tooltip「主題不能空白」；**不要 disable 下一步 button**（user 看不出原因）
- **進到 step 2 後不再可編輯 topic**——想改就 Cancel 回來（topic 改 = scope reset）

### Quiz wizard Step 2 · Scope（→ walkthrough-decisions § 3.2 Design A）

詳見上方 `### Quiz scope-confirm step（FLAGSHIP surface · design v1.5 spec lock）`區、不重畫避免 spec drift。

### Quiz wizard Step 3 · Generating [design v1.1 spec lock]

- **Header right**：amber pulse dot + `Running` + mono `14s · 3.2k tok` + danger `⏹ Cancel`（**QG5 / QG6 / QG7 全解**——header 加 Cancel + elapsed/tokens、開場有 🎓 brand banner）
- **Banner**（依序）：
  - `🎓 quiz 主題：<topic>`
  - `🚌 CodeBus 出題中… 範圍 5 頁：<paths>`
  - `🤔 <LLM 思考 thought block>`
- **Activity stream**：沿用 §02 W4 cluster 系統，但 quiz 只有 `Reading wiki` cluster（不寫 wiki、無 Writing cluster）
- **Live tail**：cluster 最底加 spinner + amber 文字「drafting question 3 of 5」+ blinking caret
- **取消 = 退回 Quiz history**——不留 partial quiz（跟 Goal 不同：Goal 中斷保留 partial wiki；quiz 沒「部分跑完」的價值）
- **QGEN1 marker filter** 在 ActivityStreamItem render 時 detect `[CODEBUS_*]` pattern、替換 user-friendly（thought block renderer 責任、跟 wizard layout 解耦）

### Quiz wizard Step 4a · Answering Pending [design v1.1 spec lock]

- **Header**：title 變 `Quiz: <scope>` (scope 用 mono dim)、step dots 同 layout、右側 `Q1 / 5` mono + Cancel
- **Question 區**：`Q1.` mono dim（22px/600）+ kind 標籤 `multiple-choice · single answer` dim + question text 22px/600
- **Choice rows**：`[A]` letter chip + radio circle + label + optional tag——每 row 44px tall
- **Footer**：左側 kbd hints `[A][B][C][D] to select · [⏎] to submit`、右側 Submit button（**未選時 disabled、opacity 0.5；選後變 amber primary**）

### Quiz wizard Step 4b · Answering Reviewing [design v1.1 spec lock]

- **Header**：同 Pending、右側 `Q2 / 5`
- **Choice rows**：
  - **wrong row**（user 選錯）：red border + `✕` radio + red `your answer` tag
  - **correct row**：green border + `✓` radio + green `correct` tag
  - **其他 rows**：fade `opacity: 0.55`
- **Citation block**（QA6 spec、放 choice list 下方）：2px amber left border + bg-raised + amber `"` opening glyph + cited quote + `cite-link` dashed-amber-mono wikilink
- **Footer**：左 `← 回 wiki page` plain link（QA8）、右 `→ to advance` kbd hint + amber primary `Next: Q3 →`
- **最後一題答完**：Next 文字換「看結果 →」、進 completion summary

### Quiz wizard Step 4c · Completion summary [design v1.1 spec lock]

- **Header**：左上 plain link 「← 回 history」、title 變 `Quiz <topic> · result` fg-secondary
- **Hero**：
  - **Fail**：lucide `XCircle` red 56px + 「沒通過 (40%)」 + 「正確率 40% · 未達 69% 門檻」
  - **Pass**：lucide `CheckCircle2` green 56px + 「通過了 (80%)」 + 「正確率 80% · 超過 69% 門檻」
- **Actions（2 button）**：
  - amber primary：「↻ 重做此份」
  - secondary：**fail = 「看錯題」 / pass = 「看過程」**（不強求對稱、語意不同）
- **Wrong list（fail 時顯示在下方）**：`wrong: Q2 · Q3 · Q5` mono fg-tertiary 12px（ambient 訊息、非 actionable）；pass 時 100% 不顯示這行
...
##### QE4 · `+ New quiz` 位置 [shared with GP2 / R6] [archived 2026-05-27]
...
##### QL6 · header「Quiz history」 + 右上 `+ New quiz` 位置 [shared GP2 / R6 / QE4] [archived 2026-05-27]
...
- `⌘K` 預設開 modal mode、不再只是 toggle
- 取代 cut 掉的 05 Cmd+K Overlay、達到 spotlight 體驗
...
#### 批次 6 · Layout reorg：CTA 搬進 content header（含 empty state visual）

- R2 / R6 / GP1 / GP2 / QE4 / QL6
- **R3 / R4 / R5 一起包**（2026-05-26 補）：empty state 三段式 visual + 英文 hardcode i18n + 大留白緩解；理由：R2 加 header row 後 R3 視覺更突兀，必須一起做
...
| `workspace-content-header-row` | 批次 6（含 R3/R4/R5 empty state visual） |
| `workspace-sidebar-rework` | 批次 7 |
| `goal-detail-hybrid-timeline-and-cancel-move` | 批次 8 |
| `quiz-fullscreen-wizard-view` | 批次 9 |
| `lobby-holistic-refresh` | 批次 10 + 批次 12（2026-05-26 合：Lobby polish + Vault 詞拿掉 + ODI-1 同 scope 都在 `src/components/lobby/*`）|
...
### ~~Blocked by design v1.1 handoff~~ → 全 unblock（2026-05-26）

Design v1.1 mock 已交付（`v1.1-mocks.html`、5 個 view spec lock）。原 blocked items 全部移到 Phase 6 actionable list（上方）。
...
**拆解依「畫面區域」分 3 個 spectra change**（2026-05-26 細拆，編號歸屬已 verify：R/GP/QE/QL = Workspace、G = Lobby、S/F1 = Sidebar）：

##### Phase 4A · `lobby-holistic-refresh`（純 04 Lobby）⭐ 建議先做

- **Lobby empty polish**：G1 vertical layout / G2 amber pill / G3 num style / G6 footer token 套 `cb-lobby-foot` / G7 density
- **G4 中文 section label**：套 `<SectionLabel>` caps 變體（Phase 2 已 build；會被 4C 的 GP3 RECENT label 引用，但無 cross-change dep）
- **Vault 詞 UI 拿掉**：G-copy-2（topbar `+ 新增` / section `最近` / drag tip）+ G-04a-1（vault card kebab + hover affordance）
- **ODI-1 Bumpy road**（locked）：04b hero idle motion，順手 land

→ 工時：1 天；scope 集中在 `src/components/lobby/*` + 少量 `App.tsx`；ROI 最高（user 第一眼感受到改變）

##### Phase 4B · `workspace-sidebar-rework`（純 01 Workspace sidebar）

- S3 drop `VAULT` section label
- S4 emoji prefix 🚏 / 📂 / 🎓
- S5 nav row 右側 mono count
- S6 active row 左 2px amber bar
- S7 sidebar 底部 settings icon + ⌘K kbd chip（drop refresh button）
- F1 BottomStrip 改 Lobby-only（settings 搬 sidebar 後 BottomStrip 失去存在意義）

→ 工時：1 天；scope 集中在 `src/components/workspace/Workspace.tsx` + sidebar 元件

##### Phase 4C · `workspace-content-header-row`（01 Workspace 右側內容區 + Goals/Quiz tab）

- **R2 + R6**：empty state content header row + `+ New goal` CTA 進 header
- **R3 + R4 + R5**：empty state 三段式 layout + 英文 hardcode i18n + 大留白緩解（**包進 4C**——R2 加 header row 後 R3 視覺更突兀，必須一起做）
- **GP1 + GP2**：populated state content header（shared with R2 / R6）
- **GP3**：`RECENT` `<SectionLabel>` caps（shared with G4 / S5）
- **QE4 + QL6**：quiz tab content header row + `+ New quiz` CTA

→ 工時：~1 天；shared header pattern 抽 reusable component；4B 完後做更順（S6 active amber bar 跟 content header h1 視覺呼應）

✅ **archived 2026-05-27**（`workspace-content-header-row` apply 完成）

- 新增 `<TabContentHeader>` 共用 component（`src/components/ui/`），Goals + Quiz 兩 tab content header row 同 component 渲染。
- Goals empty 改三段式 layout（header + 🎯 hero + 3 pill）；GOAL_EXAMPLES 英文常數移除、改走 `workspace.goals.examplePlaceholder1..3` i18n key。
- Goals populated 上方加 `RECENT` `<SectionLabel variant="caps">`。
- 新增 6 個 i18n key（headerTitle / headerSubtitle / emptyHeroTitle / emptyHeroSubtitle for Goals + headerTitle / headerSubtitle for Quiz）。
- CDP smoke 抓到 AUDIT R6「保留現有 N keyboard shortcut」其實**從未存在**——本 change scope 一併補：新增 `useNewGoalShortcut.ts` hook（bare N / 不帶 modifier / 避開 input.textarea），GoalsTab consume；chip 視覺與行為 align。
- 4B amber bar ↔ 4C h1 視覺呼應採「結構式 handshake」（L 形閱讀路徑：amber bar → h1 → CTA amber），不強制色彩 match；對應結論存於 `codebus-app/scripts/.content-header-smoke/4b-4c-visual-handshake.md`。
- CDP smoke 通過 4 種 state × 2 locale = 8 種畫面 + N shortcut + 跨 tab DOM 一致性；證據存於 `codebus-app/scripts/.content-header-smoke/`。

🟢 **Phase 4 layout structure 階段完整收尾**：4A Lobby refresh + 4B sidebar rework + 4C content header row 三 change 全 archive。Workspace 左 nav + 右內容區皆對齊 design v1 content-header-row pattern；下一步進入 **Phase 5 結構性 + behavior**（GP8 running stream tail / QNEW wizard / R7-2 Cancel 搬位 / ODI-4 pulse dot / 02c rename 等）。

##### 順序

**4A → 4B → 4C**（一週可全收）。三者畫面區域各自獨立、不衝突；4A ROI 最高先做、4C 用到 4B pattern 後做更好驗。

##### 14 個 `<SectionLabel>` 套用位置

Phase 4 三個 change 結束後，跨 4A (G4) + 4B (S3 drop 不算套) + 4C (GP3) + Phase 5/6 其餘位置（WK1 / D1 / I3 / etc.）會把 14 個位置全套完——不開獨立 change，分散到各自所屬 change 落地。
...
Design v1.1 mock 已交、5 個 view 全部 spec 完成。可動：
